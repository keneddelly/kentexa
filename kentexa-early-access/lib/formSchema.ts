...
export const registrationFormSchema = z
  .object({
    ...
    cargoCapacity: z.string().trim().optional().or(z.literal('')),
    routeType: z.string().trim().optional().or(z.literal('')),
    // seller
    ...
    readyProductCount: z
      ...
      .optional()
      .transform((v) => (v === undefined || Number.isNaN(v) ? undefined : v)),
    priceRange: z.string().trim().optional().or(z.literal('')),
    // service_provider
    travelsToCustomer: z.boolean().optional(),
    currentBookingMethod: z.string().trim().optional().or(z.literal('')),
    pricingModel: z.string().trim().optional().or(z.literal('')),
    // agent
    ...
    agentType: z.string().trim().optional().or(z.literal('')),
    dailyCapacity: z.string().trim().optional().or(z.literal('')),
    // shared: transporter / service_provider / agent
    coverageRegions: z.array(z.string()).optional(),
    // shared: business / seller
    needsDeliverySupport: z.boolean().optional(),
  ...
  .superRefine((data, ctx) => {
    if (data.accountType === AccountType.TRANSPORTER) {
      if (!data.vehicleType) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['vehicleType'], message: 'err_vehicle_type' });
      }
      if (!data.cargoCapacity) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['cargoCapacity'], message: 'err_cargo_capacity' });
      }
      if (!data.routeType) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['routeType'], message: 'err_route_type' });
      }
    }
