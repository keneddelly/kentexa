...
// Default collection fees — admin can override per city/zone
const DEFAULT_URBAN_FEE = 1500;
const DEFAULT_RURAL_FEE = 3000;  // base rural, admin sets up to 5000

@Injectable()
export class ParcelCollectionsService {
  constructor(
    @InjectRepository(ParcelCollection) private collectionRepo: Repository<ParcelCollection>,
  ...
  async createCollectionRequest(
    ...
    const saved: ParcelCollection = (await this.collectionRepo.save(collection)) as unknown as ParcelCollection;
    ...
    const parcel = await this.parcelRepo.findOne({ where: { order: { id: order.id } } });
    if (parcel) {
      await this.parcelRepo.update(parcel.id, {
        status: ParcelStatus.COLLECTION_REQUESTED,
      } as any);
      await this.collectionRepo.update(saved.id, { parcel } as any);
    }
  ...
  async getMyCollections(agent: User): Promise<ParcelCollection[]> {
    return this.collectionRepo.find({
      ...
      relations: { order: { product: true, seller: true }, seller: true, parcel: true },
  ...
  async claimCollection(collectionId: number, agent: User): Promise<ParcelCollection> {
    ...
    if (job.status !== CollectionStatus.REQUESTED) {
      throw new BadRequestException('This collection job is no longer available');
    }
  ...
  async confirmCollected(collectionId: number, agent: User, notes?: string): Promise<any> {
    const job = await this.collectionRepo.findOne({
      where: { id: collectionId },
      relations: { order: { buyer: true, seller: true }, parcel: true },
    });
    if (!job) throw new NotFoundException('Collection job not found');
    if ((job as any).agent?.id !== agent.id) throw new ForbiddenException('Not your job');
  ...
  async confirmHandedOver(collectionId: number, agent: User): Promise<any> {
    ...
    if (job.status !== CollectionStatus.COLLECTED) {
      throw new BadRequestException(`Cannot hand over — must be in collected status`);
    }
    ...
    const agentProfile = await this.agentRepo.findOne({ where: { user: { id: agent.id } } });
    if (agentProfile) {
      await this.agentRepo.update(agentProfile.id, {
        totalCollectionsCompleted: agentProfile.totalCollectionsCompleted + 1,
        totalEarningsCollections:  Number(agentProfile.totalEarningsCollections) + Number(job.collectionFee),
        pendingEarnings:           Number(agentProfile.pendingEarnings) + Number(job.collectionFee),
      } as any);
    }
  ...
  async getAllCollections(filters?: { status?: string; city?: string }) {
    ...
    return this.collectionRepo.find({
      where,
      relations: { order: { product: true }, seller: true, agent: true, parcel: true },
  ...
  async adminAssignAgent(collectionId: number, agentUserId: number): Promise<any> {
    const job   = await this.collectionRepo.findOne({ where: { id: collectionId }, relations: { order: true, seller: true } });
    const agent = await this.agentRepo.findOne({ where: { user: { id: agentUserId } } });
  ...
  async adminCancel(collectionId: number, reason: string): Promise<any> {
    ...
    if (job.parcel) {
      await this.parcelRepo.update(job.parcel.id, { status: ParcelStatus.PENDING } as any);
    }
