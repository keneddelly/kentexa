...
export type FeedFilter = 'for_you'|'following'|'nearby'|'products'|'services'|'transport'|'businesses'|'trending';
...
export class FeedService {
  ...
  // ── Publish a post ────────────────────────────────────────────────────────
  async publish(sellerId: number, dto: {
    type: string; title: string; body?: string;
    imageUrl?: string; linkedEntityType?: string;
    linkedEntityId?: number; ctaLabel?: string; expiresAt?: string;
    category?: string;
  }): Promise<BusinessFeedItem> {
    const item = await this.feedRepo.save(this.feedRepo.create({
      businessId:       sellerId,
  ...
  async recordEngagement(userId: number, postId: number, type: EngagementType): Promise<{
    toggled: boolean;
    cvsScore: number;
  }> {
    ...
    if (existing && (type === EngagementType.SAVE || type === EngagementType.SHARE)) {
      // Toggle off saves and shares
      await this.engageRepo.remove(existing);
      post.cvsScore    = Math.max(0, Number(post.cvsScore) - weight);
      if (type === EngagementType.SAVE)  post.saveCount  = Math.max(0, post.saveCount - 1);
      if (type === EngagementType.SHARE) post.shareCount = Math.max(0, post.shareCount - 1);
      toggled = false;
    } else if (!existing) {
  ...
  async getMomentStories(viewerId?: number): Promise<any[]> {
    ...
    if (viewerId) {
      const rows: { sellerId: number }[] = await this.feedRepo
        .query('SELECT "sellerId" FROM follow WHERE "followerId" = $1', [viewerId])
        .catch(() => []);
  ...
  // ── Comments ──────────────────────────────────────────────────────────────
  async addComment(userId: number, postId: number, body: string, parentId?: number): Promise<PostComment> {
    ...
    if (post && post.businessId !== userId) {
      ...
      // actionParam carries "{businessId}-feed" so Activity.js/App.js/CommerceProfile.js
      // land the tap on the Posts/Feed tab (where this post + its comment count live)
      // instead of the default Products tab. See CommerceProfile.js's pageParam parsing.
      this.notifService.notify({
        ...
        actionParam: `${post.businessId}-feed`,
        icon:        '💬',
      }).catch(() => {});
    }
  ...
  // ── Main feed — filtered ──────────────────────────────────────────────────
  async getFeed(userId: number | null, filter: FeedFilter = 'for_you', page = 1): Promise<{
    items: any[];
    total: number;
  }> {
    const limit = 15;
    ...
    switch (filter) {
      ...
      case 'trending': {
        ...
        const posts = await this.feedRepo
          ...
          .getMany();
        return { items: this.formatPosts(posts, 'trending'), total: posts.length };
      }
      ...
      case 'following': {
        ...
        const posts = await this.feedRepo
          ...
          .skip(offset).take(limit)
          .getMany();
        return { items: this.formatPosts(posts, 'following'), total: posts.length };
      }
      ...
      case 'nearby': {
        ...
        if (userId) {
          ...
          city = (u as any)?.city || u?.businessLocation?.split(',')[0]?.trim() || city;
        }
        const listings = await this.classifiedRepo
          .createQueryBuilder('c')
          .leftJoinAndSelect('c.seller', 's')
          .where("c.status = 'active'")
          .andWhere('LOWER(c.location) LIKE LOWER(:city)', { city: `%${city.split(' ')[0]}%` })
          .andWhere('c.createdAt > :cutoff', { cutoff: cutoff48h })
          .orderBy('s.reputationScore', 'DESC')
          .addOrderBy('c.createdAt', 'DESC')
          .skip(offset).take(limit)
          .getMany();
        return { items: this.formatClassifieds(listings), total: listings.length };
      }
      ...
      case 'businesses': {
        const businesses = await this.userRepo
          ...
          .skip(offset).take(limit)
          .getMany();
        return { items: this.formatBusinesses(businesses), total: businesses.length };
      }
      ...
      default: {
        ...
        const items = [
          ...this.formatPosts(followed as any, 'following'),
          ...this.formatPosts(trending as any, 'trending'),
  ...
  async getTrending(): Promise<{
    ...
    const [topPosts, fastGrowing, topBiz, topSvc] = await Promise.all([
      ...
      this.feedRepo
        ...
        .leftJoinAndSelect('f.business', 'b')
        .where('f.isActive = true')
        .andWhere('f.createdAt > :cutoff', { cutoff: new Date(Date.now() - 6*60*60*1000) })
        .orderBy('f.cvsScore', 'DESC')
        .take(5).getMany(),
    ...
    return {
      topProducts:    this.formatPosts(topPosts as any, 'trending'),
      fastestGrowing: this.formatPosts(fastGrowing as any, 'trending'),
  ...
  async getDiscoverFeed(userId?: number): Promise<{
    ...
    if (userId) {
      ...
      if (followedSellerIds.size > 0) {
        ...
        followedFeed = followed.map(f => ({
          id: 'feed-' + f.id, feedType: 'business_post',
          createdAt: f.createdAt, data: f, business: f.business,
          cvsScore: f.cvsScore, saveCount: f.saveCount,
    ...
    return {
      suggestedSellers: sellers.map(u => ({
        id: u.id, userId: u.id,
        storeName: u.storeName, name: u.name, logo: u.logo,
        coverImage: u.coverImage, businessLocation: u.businessLocation,
        reputationScore: (u as any).reputationScore || 0,
        completedOrders: (u as any).completedOrders || 0,
        rating: u.rating || 0, followersCount: u.followersCount || 0,
  ...
  async deletePost(sellerId: number, postId: number): Promise<void> {
    await this.feedRepo.update({ id: postId, businessId: sellerId }, { isActive: false });
  }

  // ── Format helpers ────────────────────────────────────────────────────────
  private formatPosts(posts: BusinessFeedItem[], feedType: string) {
    return posts.map(f => ({
      id: 'feed-' + f.id, feedType,
      createdAt: f.createdAt, data: f,
      business: (f as any).business,
      cvsScore: f.cvsScore, saveCount: f.saveCount,
      commentCount: f.commentCount, purchaseCount: f.purchaseCount,
    }));
  }

  private formatClassifieds(items: any[]) {
    return items.map(c => ({
      id: 'classified-' + c.id, feedType: 'product',
      createdAt: c.createdAt, data: c,
      business: c.seller,
    }));
  }
  ...
  private formatServices(items: any[]) {
    return items.map(s => ({
      id: 'service-' + s.id, feedType: 'service',
      createdAt: s.createdAt, data: s,
      business: s.provider,
    }));
  }
  ...
  private formatBusinesses(items: any[]) {
    return items.map(u => ({
      id: 'biz-' + u.id, feedType: 'business',
      createdAt: u.createdAt, data: u,
      business: u,
    }));
  }

  // ── Notify followers ──────────────────────────────────────────────────────
  private async notifyFollowers(sellerId: number, item: BusinessFeedItem): Promise<void> {
    ...
    const followers: { followerId: number }[] = await this.feedRepo
      .query('SELECT "followerId" FROM follow WHERE "sellerId" = $1', [sellerId])
      .catch(() => []);
    for (const f of followers) {
      this.notifService.businessFeedPost(f.followerId, bizName, item.title, sellerId).catch(() => {});
    }
  }
}