...
export class DailyBatchesService {
  ...
  private readonly DAR_ZONES: Record<string, { rank: number; keywords: string[]; label: string }> = {
    zone_central: {
      rank: 1,
      label: 'Kariakoo / Ilala / Upanga',
      keywords: ['kariakoo','ilala','upanga','kisutu','gerezani','kivukoni','lumumba'],
    },
    zone_inner: {
      rank: 2,
      label: 'Kinondoni / Sinza / Magomeni / Temeke',
      keywords: ['kinondoni','sinza','mwenge','mikocheni','magomeni','mwananyamala',
                 'tandale','manzese','kijitonyama','msasani','masaki','oyster bay',
                 'temeke','kigamboni','kurasini','mtoni',"chang'ombe"],
    },
    zone_mid: {
      rank: 3,
      label: 'Ubungo / Kimara / Mbezi / Mbagala',
      keywords: ['ubungo','kimara','makuburi','kibamba','mbezi','mbezi louis',
                 'mbezi beach','mbagala','mbagala kuu','yombo','tandika'],
    },
    zone_far: {
      rank: 4,
  ...
  private async calculateBodaFee(sellerAddress: string | null, buyerAddress: string | null): Promise<number> {
  ...
  async detectZone(deliveryAddress: string | null): Promise<DeliveryZone | null> {
    if (!deliveryAddress) return null;
    const zones = await this.zoneRepo.find({ where: { isActive: true } });
    const addressLower = deliveryAddress.toLowerCase();

    for (const zone of zones) {
      const keywords = zone.addressKeywords?.length ? zone.addressKeywords : [zone.name];
  ...
  async getOrCreateOpenBatch(): Promise<DailyBatch> {
    ...
    if (!batch) {
      ...
      this.logger.log(`Created new daily batch for ${this.dateOnly(targetDate)}`);
    }
  ...
  async createOfflineOrderAndAssign(
    ...
    if (!data.productName?.trim()) throw new BadRequestException('Product name is required');
    if (!data.amount || data.amount <= 0) throw new BadRequestException('Amount is required');
    if (!data.buyerName?.trim()) throw new BadRequestException('Buyer name is required');
    if (!data.buyerPhone?.trim()) throw new BadRequestException('Buyer phone is required');
    if (!data.deliveryAddress?.trim()) throw new BadRequestException('Delivery address is required');
    ...
    return {
      ...
      zoneName: zone.name,
      zoneAgent: zone.zoneAgent?.businessName || zone.zoneAgent?.user?.name || null,
      estimatedArrival: new Date(batch.plannedDepartureTime.getTime() + zone.etaMinutesFromDeparture * 60000),
      trackingNumber,
      parcelId: parcel.id,
  ...
  async assignOrderToBatch(orderId: number, seller: User) {
    ...
    if (order.seller?.id !== seller.id) throw new BadRequestException('Not your order');
    ...
    return {
      ...
      trackingNumber,
      parcelId: parcel.id,
      message: `Parcel assigned to ${this.dateOnly(batch.runDate as any) === this.dateOnly(new Date()) ? "today's" : "tomorrow's"} van run to ${zone.name}. Hand off at Kariakoo hub before ${new Date(batch.cutoffTime).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}.`,
    };
  }

  // ── Hub: mark parcel physically received at Kariakoo ──────────────────────
  async markReceivedAtHub(parcelId: number) {
    const parcel = await this.parcelRepo.findOne({ where: { id: parcelId }, relations: { order: { buyer: true } } });
  ...
  async getTodaysManifest() {
    ...
    const batch = await this.batchRepo.findOne({
      ...
      relations: { parcels: { order: { buyer: true, product: true }, zone: true } },
    ...
    for (const p of batch.parcels) {
      ...
      if (!zoneMap.has(zoneId)) {
        zoneMap.set(zoneId, {
          ...
          zoneAgent: p.zone.zoneAgent?.businessName || p.zone.zoneAgent?.user?.name || 'Unassigned',
      ...
      zoneMap.get(zoneId).parcels.push({
        ...
        status:          p.status,
        productName:     p.order?.product?.name || p.order?.manualProductName || '—',
        recipientName:   p.order?.recipientName || p.order?.buyer?.name || p.order?.manualBuyerName || '—',
        recipientPhone:  p.order?.phone || p.order?.buyer?.phone || p.order?.manualBuyerPhone || '—',
        deliveryAddress: p.order?.deliveryAddress,
        zoneName:        p.zone.name,
      });
    }

    const zones = Array.from(zoneMap.values()).sort((a, b) => a.routeOrder - b.routeOrder);
  ...
  async departVan(batchId: number, data: { driverName?: string; driverPhone?: string; vehicleInfo?: string }) {
    const batch = await this.batchRepo.findOne({ where: { id: batchId }, relations: { parcels: { order: { buyer: true } } } });
    if (!batch) throw new NotFoundException('Batch not found');
    if (batch.status !== BatchStatus.OPEN && batch.status !== BatchStatus.CUTOFF) {
    ...
    await this.parcelRepo
      .createQueryBuilder()
      .update(BatchParcel)
      .set({ status: BatchParcelStatus.ON_VAN } as any)
    ...
    return { success: true, message: `Van departed with ${batch.parcels.length} parcels.` };
  }
  ...
  async markZoneArrival(batchId: number, zoneId: number) {
    const result = await this.parcelRepo
      .createQueryBuilder()
      .update(BatchParcel)
      .set({ status: BatchParcelStatus.AT_ZONE, arrivedAtZoneAt: new Date() } as any)
      .where('batchId = :batchId AND zoneId = :zoneId', { batchId, zoneId })
      .execute();

    return { success: true, updated: result.affected || 0, message: 'Zone marked as arrived. Parcels ready for last-mile delivery.' };
  }
  ...
  async markParcelDelivered(parcelId: number) {
    ...
    await this.notificationsService.delivered(
      { email: parcel.order.buyer?.email, phone: parcel.order.buyer?.phone, name: parcel.order.buyer?.name },
      parcel.order.id,
      parcel.trackingNumber || `Order #${parcel.order.id}`,
    );

    return { success: true, message: 'Parcel marked delivered. Buyer notified.' };
  }
  ...
  async getParcelStatusForOrder(orderId: number) {
    ...
    return {
      ...
      zoneAgent: parcel.zone.zoneAgent?.businessName || parcel.zone.zoneAgent?.user?.name || null,
      ...
      estimatedArrival: new Date(
        new Date(parcel.batch.plannedDepartureTime).getTime() + parcel.zone.etaMinutesFromDeparture * 60000,
      ),
  ...
  async getZones() {
    return this.zoneRepo.find({ where: { isActive: true }, order: { routeOrder: 'ASC' } });
  }

  async updateZone(id: number, data: Partial<{
    name: string; routeOrder: number; etaMinutesFromDeparture: number;
    zoneAgentId: number; addressKeywords: string[]; isActive: boolean;
  }>) {
  ...
  async getDeliveryMethods(buyerAddress: string, productId: number, _unused?: any) {
    ...
    const DAR_AREA_KEYWORDS = [
      // City center & Ilala
      'kariakoo','ilala','kisutu','upanga','gerezani','kivukoni',
      // Kinondoni
      'kinondoni','mikocheni','mwenge','sinza','kijitonyama',
      'mwananyamala','magomeni','tandale','manzese','kigogo',
      // Ubungo
      'ubungo','kimara','makuburi','kibamba',
      // Mbezi area
      'mbezi','mbezi louis','mbezi beach',
      // Temeke south
      'temeke','mbagala','mbagala kuu','kigamboni',
      "chang'ombe",'mtoni','kurasini','tandika','yombo',
      // Far north
      'bunju','bunju a','bunju b','mbweni','tegeta','boko','kunduchi',
      // Msasani peninsula
      'msasani','masaki','oyster bay',
      // General
      'dar es salaam','salaam','dsm',
    ];

    const buyerInDar = DAR_AREA_KEYWORDS.some(kw => lower.includes(kw)) ||
    ...
    try {
      ...
      this.logger.error('getDeliveryMethods product fetch failed: ' + e.message);
    }
    ...
    this.logger.log(`DeliveryMethods: address="${buyerAddress}" buyerInDar=${buyerInDar} sellerCity="${sellerCity}" isSameCity=${isSameCity} zones=${zones.length}`);

    // Find matching batch zone from DB keywords
    let matchedZone: DeliveryZone | null = null;
    for (const zone of zones) {
      const keywords = (zone.addressKeywords || [zone.name]).map(k => k.toLowerCase().trim());
    ...
    if (!isSameCity) {
      ...
      try {
        ...
        if (buyerCityGuess) {
          routeInfo = await this.routeRepo.findOne({
            where: { originCity: origin, destinationCity: buyerCityGuess, isActive: true },
          }).catch(() => null);
        }
    ...
    const sameZone   = this.detectZoneRank(sellerCity) === this.detectZoneRank(buyerAddress);
    ...
    if (matchedZone) {
      ...
      const etaHour  = deptHour + Math.floor(matchedZone.etaMinutesFromDeparture / 60);
  ...
  async getBodaFeeSuggestions(sellerAddress?: string) {
    ...
    const zones = [
      { rank: 1, label: 'Kariakoo / Ilala / Upanga',  note: 'Eneo la kati — dakika 5-15' },
      { rank: 2, label: 'Kinondoni / Sinza / Temeke', note: 'Kati — dakika 15-30' },
      { rank: 3, label: 'Ubungo / Kimara / Mbagala',  note: 'Umbali wa kati — dakika 30-50' },
      { rank: 4, label: 'Bunju / Tegeta / Boko',      note: 'Mbali sana — dakika 60+' },
    ];
  ...
  async autoCutoffCheck() {
    try {
      ...
      const result = await this.batchRepo
        .createQueryBuilder()
        .update(DailyBatch)
        .set({ status: BatchStatus.CUTOFF } as any)
        .where('status = :status', { status: BatchStatus.OPEN })
        .andWhere('cutoffTime < :now', { now })
        .execute();

      if (result.affected && result.affected > 0) {
        this.logger.log(`Auto-marked ${result.affected} batch(es) as cutoff passed`);
      }
    } catch (err) {
  ...
  async checkStaleEscrow() {
    try {
      ...
      const staleOrders = await this.orderRepo
        ...
        .andWhere("o.status NOT IN (:...done)", { done: ['confirmed', 'cancelled', 'refunded', 'delivered'] })
      ...
      const totalStaleAmount = staleOrders.reduce((s, o) => s + Number(o.totalAmount || 0), 0);
...
`
      ...
      try {
        await this.notificationsService.orderPlaced(
          { email: process.env.ADMIN_EMAIL || 'thekeneddelly@gmail.com', name: 'KenteXa Admin' },
          staleOrders[0].id,
  ...
  private guessCityFromAddress(address: string): string | null {
    ...
    for (const city of CITIES) {
      if (lower.includes(city)) return city.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
    }
    return null;
  }
  ...
  async autoReleaseEscrow() {
    try {
      ...
      console.log(`[Auto-Release] Processing ${overdueOrders.length} overdue orders...`);
      ...
      for (const order of overdueOrders) {
        try {
          ...
          await this.notificationsService.orderCompleted(
            { email: order.seller?.email, phone: order.seller?.phone, name: order.seller?.name || 'Muuzaji' },
            { email: order.buyer?.email,  phone: order.buyer?.phone,  name: order.buyer?.name  || 'Mteja'   },
            order.id,
            Number(order.sellerAmount || 0),
          ).catch(() => {});

          console.log(`[Auto-Release] Order #${order.id} released to seller ${order.seller?.name}`);
        } catch (err) {
          console.error(`[Auto-Release] Failed for order #${order.id}:`, err.message);
        }
      }

      console.log(`[Auto-Release] Done. Released ${overdueOrders.length} orders.`);
    } catch (err) {
