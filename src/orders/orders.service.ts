...
import {
  Order, OrderStatus, PaymentStatus, EscrowStatus, PayoutStatus,
...
const CATEGORY_COMMISSION: Record<string, number> = {
  electronics: 10, fashion: 12, vehicles: 5, food: 10,
  home_garden: 10, health_beauty: 10, baby_kids: 10, sports: 10,
  agriculture: 8,  security: 8,  books: 10, arts: 10,
  general: 10, property: 0, services: 0,
};
...
const calcCommission = (baseAmount: number, category: string, deliveryFee = 0) => {
...
export class OrdersService {
  constructor(
    ...
    @InjectRepository(ParcelTracking) private parcelTrackingRepo: Repository<ParcelTracking>,
  ...
  async create(dto: CreateOrderDto, user: User) {
    ...
    const chosenMethod   = (dto as any).shippingMethod || product.shippingMethod || 'agent';
    ...
    const isIntraCity    = ['boda', 'kentexa_delivery'].includes(chosenMethod);
    ...
    if (chosenMethod === 'boda') {
      ...
      deliveryFee = (dto as any).deliveryFee !== undefined
    ...
    const totalAmount    = (basePrice + deliveryFee) * dto.quantity + collectionFee;
    ...
    const commission     = calcCommission(baseAmount, category, deliveryAmount);
    ...
    const order = this.repo.create({
      ...
      sellerPickupAddress:  needsCollection ? ((dto as any).sellerPickupAddress || null) : null,
    } as any);
    ...
    await this.repo.update(saved.id, { trackingNumber: `KTX-ORD-${saved.id}` } as any);
    ...
    try {
      ...
      console.error(`Invoice creation failed for order #${saved.id}:`, err.message, err.stack);
    }
    ...
    if (chosenMethod === 'kentexa_delivery') {
      try {
        ...
        batchInfo = { message: 'Order queued for KenteXa batch delivery', orderId: saved.id };
      } catch (err) {
    ...
    if (needsCollection) {
      try {
        const sellerCity = (product.seller as any)?.businessLocation?.split(',')[0]?.trim()
        ...
        console.error(`Collection request failed for order #${saved.id}:`, err.message);
      }
    }
    ...
    if (chosenMethod === 'agent') {
      try {
        const sellerCity = (product.seller as any)?.businessLocation
          ?.split(',')[0]?.trim() || 'Dar es Salaam';
        const originAgent = await this.superAgentRepo.findOne({
          where: { city: sellerCity, status: 'active' as any },
          order: { rating: 'DESC' as any, totalParcelsDelivered: 'DESC' as any },
        });
        if (originAgent) {
          await this.repo.update(saved.id, { originSuperAgentId: originAgent.id } as any);
        }

        // Also look up the destination city Super Agent based on delivery address
        const destCity = dto.deliveryAddress?.split(',')
          .map((p: string) => p.trim())
        ...
        if (destCity && destCity !== sellerCity) {
          ...
          if (destAgent) {
            await this.repo.update(saved.id, { destinationSuperAgentId: destAgent.id } as any);
          }
        }
      } catch (err) {
        console.error('Super Agent auto-assign failed (non-critical):', err.message);
      }
    }
    ...
    try {
      const sellerId: number | undefined = (saved.seller as any)?.id ?? (product.seller as any)?.id;
      if (sellerId) {
        await this.businessCustomerService.upsertFromOrder({
          sellerId,
          userId:      user.id || null,
          name:        (dto as any).recipientName || user.name || (dto as any).manualBuyerName || 'Mteja',
      ...
      console.error('BusinessCustomer upsert failed (non-critical):', e.message);
    }
  ...
  async uploadShippingProof(orderId: number, seller: User, data: {
    ...
    if (order.seller?.id !== seller.id) throw new ForbiddenException('Not your order');
    if (![OrderStatus.PAID, OrderStatus.PREPARING].includes(order.status as any)) {
      throw new BadRequestException('Order must be paid before shipping');
    }

    const chosenMethod = data.shippingMethod || order.shippingMethod || 'direct';
  ...
  async markShipped(orderId: number, seller: User) {
    const order = await this.repo.findOne({ where: { id: orderId }, relations: { seller: true, buyer: true } });
    if (!order) throw new NotFoundException('Order not found');
    if (order.seller?.id !== seller.id) throw new ForbiddenException('Not your order');
    if (order.status !== OrderStatus.PREPARING) throw new BadRequestException('Upload shipping proof first');
  ...
  async sellerHandToSuperAgent(orderId: number, seller: User, data: {
    superAgentCity: string;
    notes?: string;
  }) {
    const order = await this.repo.findOne({ where: { id: orderId }, relations: { seller: true } });
    if (!order) throw new NotFoundException('Order not found');
    if (order.seller?.id !== seller.id) throw new ForbiddenException('Not your order');
    if (order.status !== OrderStatus.PAID) throw new BadRequestException('Order must be paid');

    await this.repo.update(orderId, {
      status:        OrderStatus.PREPARING,
      shippingMethod: 'agent',
      shippingNote:   data.notes || `Handed to Super Agent in ${data.superAgentCity}`,
    } as any);
  ...
  async lookupForAgent(orderId: number) {
    ...
    if (!['paid', 'preparing'].includes(order.status)) {
      throw new BadRequestException(`Order status is ${order.status}. Must be paid or preparing.`);
    }

    const addressParts = (order.deliveryAddress || '').split(',').map(s => s.trim()).filter(Boolean);
    const guessedCity  = addressParts[0] || null;

    return {
      orderId:         order.id,
      productName:     order.product?.name || (order as any).manualProductName || '—',
      recipientName:   (order as any).recipientName || (order as any).manualBuyerName || order.buyer?.name || '—',
      buyerName:       order.buyer?.name  || (order as any).manualBuyerName || '—',
      buyerPhone:      order.phone || order.buyer?.phone || (order as any).manualBuyerPhone || '—',
      ...
      isOffline:       ['offline','offline_intercity','seller_shipment'].includes(order.source as string),
  ...
  async superAgentReceiveOrder(
    ...
    if (isManualOrder) {
      ...
      actualFee = data.actualShippingFee != null && data.actualShippingFee !== 0
      ...
      if (feeDifference > threshold) {
        const excessOverThreshold = parseFloat((feeDifference - threshold).toFixed(2));
        sellerAmountAdjustment = -excessOverThreshold;
        shippingFeeFlagged = true;
        shippingFeeNote = `Shipping fee was TZS ${actualFee.toLocaleString()} vs seller's estimate of TZS ${estimatedFee.toLocaleString()} `
        ...
        shippingFeeNote = `Shipping fee adjusted from TZS ${estimatedFee.toLocaleString()} to TZS ${actualFee.toLocaleString()} `
          + `(${feeDifference > 0 ? '+' : ''}${feeDifference.toLocaleString()}) — within acceptable range, absorbed by KenteXa.`;
      }

      newSellerAmount = sellerAmountAdjustment !== 0
        ? parseFloat((Number(order.sellerAmount || 0) + sellerAmountAdjustment).toFixed(2))
    ...
    const orderUpdate: any = {
      ...
      shippingMethod:       'agent',
      shippingNote:         data.notes || `Received at ${data.originCity} Super Agent hub`,
    ...
    const superAgentProfile = await this.superAgentRepo.findOne({ where: { user: { id: superAgent.id } } });
    ...
    if (superAgentProfile) {
      if (isManualOrder) {
        ...
        const agentCommissionRate = Number((superAgentProfile as any).commissionRate || 0);
        superAgentEarnings = parseFloat(((actualFee * agentCommissionRate) / 100).toFixed(2));
    ...
    try {
      await this.parcelRepo.save(
        this.parcelRepo.create({
          ...
          estimatedShippingFee: isManualOrder ? null : Number((order as any).deliveryFeeAmount || 0),
          actualShippingFee:    actualFee,
          shippingMargin:       isManualOrder ? null : parseFloat(((Number((order as any).deliveryFeeAmount || 0)) - actualFee).toFixed(2)),
          superAgentEarnings,
  ...
  async agentMarkReceived(orderId: number, agent: User, note?: string) {
    const order = await this.repo.findOne({ where: { id: orderId }, relations: { buyer: true } });
    if (!order) throw new NotFoundException('Order not found');
    if (![OrderStatus.IN_TRANSIT, OrderStatus.PREPARING].includes(order.status)) {
    ...
    try {
      const agentUser = await this.userRepo?.findOne?.({ where: { id: agent.id } });
      if (agentUser) {
  ...
  async buyerConfirm(orderId: number, buyer: User) {
    const order = await this.repo.findOne({ where: { id: orderId }, relations: { buyer: true, seller: true, product: true } });
    if (!order) throw new NotFoundException('Order not found');
    if (!order.buyer || order.buyer.id !== buyer.id) throw new ForbiddenException('Not your order');
    if (![OrderStatus.DELIVERED, OrderStatus.READY_PICKUP, OrderStatus.IN_TRANSIT].includes(order.status)) {
    ...
    try {
      const ord = await this.repo.findOne({ where: { id: orderId }, relations: { buyer: true, seller: true } });
      if (ord?.buyer?.id) await this.reputationService.award(ord.buyer.id, ReputationEventType.ORDER_COMPLETED, { sourceEntityType: 'order', sourceEntityId: orderId });
      if (ord?.seller?.id) await this.reputationService.award(ord.seller.id, ReputationEventType.ORDER_COMPLETED, { sourceEntityType: 'order', sourceEntityId: orderId });
    } catch { /* non-critical */ }
    ...
    if (order.trackingNumber) {
      try {
        const parcel = await this.parcelRepo.findOne({ where: { trackingNumber: order.trackingNumber } });
        if (parcel && parcel.status !== ParcelStatus.DELIVERED) {
          ...
          if (parcel.localAgentId) {
            const localAgent = await this.agentRepo.findOne({ where: { user: { id: Number(parcel.localAgentId) } } });
            if (localAgent) {
              const alreadyCredited = await this.agentTransactionRepo.findOne({
                where: { transactionReference: order.trackingNumber, paymentMethod: 'last_mile_delivery' },
              });
              if (!alreadyCredited) {
                const platformFee = Number(order.platformFeeAmount || 0);
                if (platformFee > 0) {
                  const commissionRate = Number(localAgent.commissionRate ?? 2.5);
                  const commission     = parseFloat(((platformFee * commissionRate) / 100).toFixed(2));
                  if (commission > 0) {
        ...
        console.error('Failed to sync parcel status on buyer confirm:', err.message);
      }
    }

    return { message: 'Confirmed! Payment will be released to seller within 24 hours.' };
  }
  ...
  async generateConfirmationLink(orderId: number, seller: User): Promise<{
    confirmationUrl: string; buyerPhone: string | null;
  }> {
    const order = await this.repo.findOne({
      where: { id: orderId },
      relations: { seller: true, buyer: true, product: true },
    ...
    const isSeller = (order.seller as any)?.id === seller.id ||
    ...
    const buyerPhone = order.buyer?.phone || (order as any).manualBuyerPhone ||
                       (order as any).phone || (order as any).recipientPhone || null;
    const buyerName  = order.buyer?.name  || (order as any).manualBuyerName  ||
                       (order as any).recipientName || 'Mteja';
    const productName = order.product?.name || (order as any).manualProductName ||
                        (order as any).description || 'bidhaa yako';
    const storeName   = (order.seller as any)?.storeName ||
                        (order.seller as any)?.businessName || seller.name;
  ...
  async getOrderByConfirmationToken(token: string) {
    ...
    if (!order) throw new NotFoundException('Kiungo hakipatikani au kimekwisha muda');

    const expiry = (order as any).confirmationTokenExpiry;
    if (expiry && new Date() > new Date(expiry)) {
      throw new BadRequestException('Kiungo kimekwisha muda. Omba muuzaji akutumie kipya.');
    }
    if (order.status === OrderStatus.COMPLETED) {
      return {
        alreadyConfirmed: true,
        orderId: order.id,
        productName: order.product?.name || (order as any).manualProductName || 'Bidhaa',
        storeName:   (order.seller as any)?.storeName || (order.seller as any)?.businessName || 'Muuzaji',
    ...
    return {
      ...
      storeName:    (order.seller as any)?.storeName || (order.seller as any)?.businessName || 'Muuzaji',
      sellerPhone:  order.seller?.phone  || (order.seller as any)?.user?.phone || null,
  ...
  async confirmViaToken(token: string, data: {
    confirmed:  boolean;
    rating?:    number;   // 1-5
    review?:    string;
    ...
    if (data.confirmed) {
      // ── CONFIRMED ─────────────────────────────────────────────────────────
      const isOnlineOrder = (order as any).source === 'online' ||
      ...
      // Notify seller
      const sellerPhone = order.seller?.phone || (order.seller as any)?.user?.phone;
      const productName = order.product?.name || (order as any).manualProductName || 'Bidhaa';
      const stars = '⭐'.repeat(data.rating || 0);
      const sellerId = (order.seller as any)?.id;
      // In-app notification
      if (sellerId) {
        await this.inAppNotif.orderConfirmed(sellerId, order.id, productName,
          data.rating || undefined, data.review || undefined);
        if (isOnlineOrder) {
          await this.inAppNotif.payoutReleasedById(sellerId,
            Number(order.sellerAmount || 0), order.id);
        }
        if (data.rating) {
          await this.inAppNotif.reviewReceived(sellerId, data.rating, productName);
        }
      }
      ...
      if (order.product?.id) {
        await this.productsService.incrementSalesCount(order.product.id).catch(() => {});
      }

      const user_confirming_id = order.buyer?.id || null;

      // ── Add product review if buyer left one ───────────────────────────────// product review handled client-side
      ...
      // Notify admin
      const sellerPhoneD = order.seller?.phone || (order.seller as any)?.user?.phone;
      const sellerIdD = (order.seller as any)?.id;
      if (sellerIdD) {
        await this.inAppNotif.disputeRaisedById(sellerIdD, order.id,
          order.trackingNumber || String(order.id));
      }
      const sellerPhone = sellerPhoneD;
      if (sellerPhone) {
        await this.smsService.sendSms(sellerPhone,
      ...
      return {
        success: false,
        message: 'Tatizo limeripotiwa. KenteXa itawasiliana nawe hivi karibuni.',
      };
    }
  }
  ...
  async autoConfirmDeliveredOrders() {
    ...
    for (const order of deliveredOrders) {
      ...
      const isIntercity = (order as any).source === 'intercity' ||
  ...
  // ── Buyer: Raise dispute ──────────────────────────────────────────────────
  async raiseDispute(orderId: number, buyer: User, data: { reason: string; image?: string }) {
    const order = await this.repo.findOne({ where: { id: orderId }, relations: { buyer: true } });
    if (!order) throw new NotFoundException('Order not found');
    if (!order.buyer || order.buyer.id !== buyer.id) throw new ForbiddenException('Not your order');
    if (![OrderStatus.DELIVERED, OrderStatus.READY_PICKUP, OrderStatus.IN_TRANSIT].includes(order.status)) {
  ...
  async resolveDispute(orderId: number, admin: User, data: { resolution: string; favour: 'buyer' | 'seller' }) {
    const order = await this.repo.findOne({ where: { id: orderId } });
    if (!order) throw new NotFoundException('Order not found');
    if (order.status !== OrderStatus.DISPUTED) throw new BadRequestException('No active dispute');
  ...
  async findMyOrders(user: User): Promise<Order[]> {
    return this.repo.find({ where: { buyer: { id: user.id } }, order: { createdAt: 'DESC' } });
  }
  ...
  async getFinancialSummary() {
    ...
    const pendingRelease = online.filter((o: any) =>
      !['confirmed','cancelled','refunded'].includes(o.status)
    );
    ...
    const totalEscrowHeld     = pendingRelease.reduce((s, o) => s + Number(o.sellerAmount || 0), 0);
    const totalPlatformFees   = completedOnline.reduce((s, o) => s + Number(o.platformFeeAmount || 0), 0);
    const totalOrderVolume    = online.reduce((s, o) => s + Number(o.totalAmount || 0), 0);
    const totalPendingPayouts = pendingRelease.reduce((s, o) => s + Number(o.sellerAmount || 0), 0);

    // This month
    const thisMonth = online.filter(o => new Date(o.createdAt) >= startOfMonth);
    const monthFees = thisMonth.filter((o: any) => ['confirmed','delivered'].includes(o.status))
      .reduce((s, o) => s + Number(o.platformFeeAmount || 0), 0);
    const monthVolume = thisMonth.reduce((s, o) => s + Number(o.totalAmount || 0), 0);

    // This week
    const thisWeek = online.filter(o => new Date(o.createdAt) >= startOfWeek);
    const weekFees = thisWeek.filter((o: any) => ['confirmed','delivered'].includes(o.status))
      .reduce((s, o) => s + Number(o.platformFeeAmount || 0), 0);
    const weekVolume = thisWeek.reduce((s, o) => s + Number(o.totalAmount || 0), 0);
  ...
  async rateSellerForOrder(orderId: number, buyer: User, rating: number, comment?: string) {
    const order = await this.repo.findOne({
      where: { id: orderId },
      relations: { seller: true, buyer: true },
    });
    if (!order) throw new NotFoundException('Order not found');
    if (order.buyer?.id !== buyer.id) throw new ForbiddenException('Not your order');
    ...
    // Rolling weighted average — keeps accuracy without storing every rating
    const currentRating = Number(seller.rating || 5.0);
    const totalRatings  = Number(seller.reviewsCount || 0);
    const newTotal      = totalRatings + 1;
    const newRating     = parseFloat(((currentRating * totalRatings + rating) / newTotal).toFixed(2));

    await this.repo.manager.getRepository('User' as any).update(seller.id, {
      rating:       newRating,
    ...
    return { success: true, newRating, message: 'Ukadiriaji umepokewa. Asante!' };
  }
  ...
  async cancel(id: number, user: User) {
    const order = await this.repo.findOne({ where: { id } });
    if (!order) throw new NotFoundException('Order not found');
    if (!order.buyer || (order.buyer.id !== user.id && user.role !== UserRole.ADMIN)) throw new ForbiddenException('Access denied');
    if (order.status !== OrderStatus.PENDING_PAYMENT) throw new BadRequestException('Only pending orders can be cancelled');
    order.status = OrderStatus.CANCELLED;
    await this.repo.save(order);
    try { if (order.product) await this.productsService.decreaseStock(order.product.id, -order.quantity); } catch {}
  ...
  async getDeliveryMethods(buyerAddress: string, productId: number) {
    ...
    const DAR_KEYWORDS = [
      'kariakoo','ilala','kisutu','upanga','kinondoni','mikocheni','mwenge',
      'sinza','ubungo','kimara','mbezi','temeke','mbagala','kigamboni',
      'bunju','tegeta','boko','kunduchi','msasani','masaki','oyster bay',
      'dar es salaam','dar','dsm','mwananyamala','magomeni','tandale',
      'manzese','kijitonyama','yombo','tandika','mtoni','kurasini',
      'mbweni','mbezi louis','mbezi beach','kibamba',"chang'ombe",
    ];
    ...
    const BATCH_ZONES: Record<string, string[]> = {
      ...
      Kinondoni: ['kinondoni','mwenge','mikocheni','sinza','msasani','masaki','oyster bay'],
    };
    ...
    for (const [zone, keywords] of Object.entries(BATCH_ZONES)) {
      if (keywords.some(kw => lower.includes(kw))) { batchZone = zone; break; }
    }
    ...
    if (!isSameCity) {
      return {
        ...
        batchZone: null,
        methods: [{
          key: 'agent', label: 'KenteXa Super Agent', icon: '🏢',
          fee: deliveryFee,
    ...
    if (batchZone) {
      methods.push({
        key: 'kentexa_delivery', label: 'KenteXa Delivery', icon: '🚐',
        fee: BATCH_FEE,
  ...
  async createOnBehalf(seller: User, dto: {
    productId:       number;
    quantity:        number;
    buyerName:       string;
    buyerPhone:      string;
    ...
    if (product.seller?.id !== seller.id) throw new ForbiddenException('Not your product');
    if (!product.isAvailable) throw new BadRequestException('Product not available');
    if (product.stock < dto.quantity) throw new BadRequestException('Insufficient stock');
    ...
    const baseAmount   = basePrice * dto.quantity;
    const deliveryAmount = deliveryFee * dto.quantity;
    const chosenMethod = dto.shippingMethod || product.shippingMethod || 'agent';
    ...
    const saved = await this.repo.save(order as any) as any;
    ...
    await this.repo.update(saved.id, { trackingNumber } as any);

    // Send SMS to customer with tracking
    try {
      const msg =
...
` +
      ...
      await this.notificationsService.orderPlaced({ phone: dto.buyerPhone, email: null, name: dto.buyerName }, saved.id, product.name, totalAmount);
    } catch {}
  ...
  async adminChangeStatus(orderId: number, status: string, admin: User) {
    ...
    if (status === 'confirmed' || status === 'delivered') {
      await this.repo.update(orderId, { escrowStatus: 'release_requested' as any }).catch(() => {});
    }
  ...
  async findByTrackingNumber(trackingNumber: string) {
    const order = await this.repo.findOne({
      ...
      select: { id: true, trackingNumber: true, shippingMethod: true, status: true },
    });
    if (!order) throw new NotFoundException(`Tracking number ${trackingNumber} not found`);
    return { id: order.id, trackingNumber: order.trackingNumber, shippingMethod: order.shippingMethod };
  }
