...
export class NotificationsService {
  ...
  private async safeEmail(fn: () => Promise<any>, label: string) {
    ...
    catch (e) { this.logger.warn(`❌ Email failed [${label}]: ${e.message}`); }
  }
  ...
  private async safeSms(fn: () => Promise<any>, label: string) {
    ...
    catch (e) { this.logger.warn(`❌ SMS failed [${label}]: ${e.message}`); }
  }
  ...
  // ════════════════════════════════════════════════════════════════════════
  async orderPlaced(buyer: NotifyTarget, orderId: number, productName: string, amount: number) {
    if (buyer.email) {
      await this.safeEmail(
        () => this.mailService.sendOrderConfirmation(buyer.email!, buyer.name || 'Customer', orderId, amount, productName),
        'orderPlaced-email',
      );
    }
  }
  ...
  // ════════════════════════════════════════════════════════════════════════
  async orderPaid(buyer: NotifyTarget, seller: NotifyTarget, orderId: number, productName: string, amount: number) {
    this.logger.log(`🔔 orderPaid() called for order #${orderId} — buyer.email=${buyer.email}, buyer.phone=${buyer.phone}, seller.email=${seller.email}, seller.phone=${seller.phone}`);
    // Buyer: SMS (allowed) + Email (if available)
    if (buyer.phone) {
      await this.safeSms(
        () => this.smsService.sendSms(buyer.phone!, `KenteXa: Malipo yako ya Order #${orderId} (TZS ${amount.toLocaleString()}) yamethibitishwa. Muuzaji ataandaa bidhaa yako hivi karibuni.`),
        'orderPaid-buyer-sms',
      );
    }
    if (buyer.email) {
    ...
    if (seller.phone) {
      await this.safeSms(
        () => this.smsService.sendSms(seller.phone!, `KenteXa: Umepokea agizo jipya #${orderId} - ${productName} (TZS ${amount.toLocaleString()}). Tafadhali andaa usafirishaji.`),
        'orderPaid-seller-sms',
      );
    }
    if (seller.email) {
      await this.safeEmail(
        () => this.mailService.sendOrderConfirmation(seller.email!, seller.name || 'Seller', orderId, amount, productName),
  ...
  // ════════════════════════════════════════════════════════════════════════
  async shippingProofUploaded(buyer: NotifyTarget, orderId: number, trackingNumber: string, courierName: string) {
    if (buyer.email) {
      await this.safeEmail(
  ...
  // ════════════════════════════════════════════════════════════════════════
  async parcelDispatched(buyer: NotifyTarget, trackingNumber: string, originCity: string, destinationCity: string) {
    if (buyer.email) {
      await this.safeEmail(
        () => this.mailService.sendGenericUpdate(
          buyer.email!,
  ...
  // ════════════════════════════════════════════════════════════════════════
  async delivered(buyer: NotifyTarget, orderId: number, trackingNumber: string) {
    if (buyer.phone) {
      await this.safeSms(
        () => this.smsService.sendSms(buyer.phone!, `KenteXa: Bidhaa yako Order #${orderId} (${trackingNumber}) imefika. Tafadhali ithibitishe kwenye app au tovuti ili tutoe malipo kwa muuzaji.`),
        'delivered-buyer-sms',
      );
    }
    if (buyer.email) {
  ...
  // ════════════════════════════════════════════════════════════════════════
  async collectionClaimed(seller: NotifyTarget, orderId: number, agentName: string, agentPhone: string) {
    if (seller.phone) {
      await this.safeSms(
  ...
  // ════════════════════════════════════════════════════════════════════════
  async parcelCollected(seller: NotifyTarget, buyer: NotifyTarget, orderId: number, trackingNumber: string) {
    if (seller.phone) {
      await this.safeSms(
        () => this.smsService.sendSms(
          seller.phone!,
  ...
  // ════════════════════════════════════════════════════════════════════════
  async collectionAtHub(seller: NotifyTarget, buyer: NotifyTarget, orderId: number, city: string) {
    if (seller.phone) {
      await this.safeSms(
        () => this.smsService.sendSms(
          seller.phone!,
  ...
  // ════════════════════════════════════════════════════════════════════════
  async classifiedInvoicePaid(buyer: NotifyTarget, seller: NotifyTarget, invoiceNumber: string, amount: number) {
    if (buyer.phone) {
      await this.safeSms(
        () => this.smsService.sendSms(buyer.phone!, `KenteXa: Malipo ya ankara ${invoiceNumber} (TZS ${amount.toLocaleString()}) yamethibitishwa.`),
        'classifiedPaid-buyer-sms',
      );
    }
    if (seller.phone) {
      await this.safeSms(
        () => this.smsService.sendSms(seller.phone!, `KenteXa: Ankara ${invoiceNumber} imelipwa - TZS ${amount.toLocaleString()}. Chagua njia ya usafirishaji kwenye dashibodi yako.`),
        'classifiedPaid-seller-sms',
      );
    }
    if (buyer.email) {
      await this.safeEmail(
        () => this.mailService.sendGenericUpdate(buyer.email!, buyer.name || 'Customer', 'Payment confirmed', `Invoice ${invoiceNumber} (TZS ${amount.toLocaleString()}) has been paid.`),
        'classifiedPaid-buyer-email',
      );
    }
    if (seller.email) {
      await this.safeEmail(
        () => this.mailService.sendGenericUpdate(seller.email!, seller.name || 'Seller', 'Invoice paid!', `Invoice ${invoiceNumber} (TZS ${amount.toLocaleString()}) has been paid. Set a shipping method on your dashboard.`),
