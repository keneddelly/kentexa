...
export class AgentsService {
  ...
  async register(user: User, dto: {
    ...
    address?: string; region?: string; district?: string;
    idNumber?: string; idType?: string; idPhotoUrl?: string;
    agentType?:          string;
    maxWeightKg?:        number;
    vehicleDescription?: string;
  }) {
    const existing = await this.agentRepo.findOne({ where: { user: { id: user.id } } });
    if (existing) {
      if (existing.status === AgentStatus.PENDING)  throw new BadRequestException('Ombi lako lipo tayari chini ya mapitio.');
      if (existing.status === AgentStatus.APPROVED) throw new BadRequestException('Wewe ni wakala tayari.');
      await this.agentRepo.update(existing.id, { ...dto, status: AgentStatus.PENDING, rejectionReason: null } as any);
      return { message: 'Ombi lako jipya limewasilishwa. Tutawasiliana nawe.' };
    }
    const agent = this.agentRepo.create({
      user, fullName: dto.fullName, phone: dto.phone,
      city: dto.city || null, address: dto.address || null,
      region: dto.region || null, district: dto.district || null,
      idNumber: dto.idNumber || null, idType: dto.idType || null,
  ...
  async getMyProfile(user: User) {
    const agent = await this.agentRepo.findOne({ where: { user: { id: user.id } } });
    if (!agent) return null;
    return {
      id: agent.id, status: agent.status, fullName: agent.fullName,
      phone: agent.phone, city: agent.city, district: agent.district,
      region: agent.region, agentCode: agent.agentCode, tier: agent.tier,
  ...
  findByCity(city: string) {
    return this.agentRepo.find({
      ...
      relations: { user: true },
      order: { rating: 'DESC' as any, totalDeliveriesCompleted: 'DESC' as any },
    }).then(agents => agents
      .filter(a =>
  ...
  async findAll(filters?: { status?: string; city?: string }) {
    ...
    return agents
      ...
      .map(a => ({
        id: a.id, userId: a.user?.id,
        fullName: a.fullName, phone: a.phone,
        city: a.city, district: a.district, region: a.region,
        agentCode: a.agentCode, status: a.status, tier: a.tier,
        idNumber: a.idNumber, idType: a.idType, idPhotoUrl: a.idPhotoUrl,
        address: a.address,
  ...
  async approve(agentId: number) {
    const agent = await this.agentRepo.findOne({
      where: { id: agentId }, relations: { user: true },
    });
    if (!agent) throw new NotFoundException('Wakala hapatikani');
    const cityCode = (agent.city || agent.district || 'KTX').slice(0, 3).toUpperCase().replace(/\s/g, '');
    const agentCode = `KTX-${cityCode}-${String(agentId).padStart(3, '0')}`;
    await this.agentRepo.update(agentId, {
      status: AgentStatus.APPROVED, agentCode, rejectionReason: null,
    } as any);
    if (agent.user) await this.userRepo.update(agent.user.id, { role: 'agent' as any });
  ...
  async reject(agentId: number, reason: string) {
    ...
    if (!agent) throw new NotFoundException('Wakala hapatikani');
    await this.agentRepo.update(agentId, {
      status: AgentStatus.REJECTED, rejectionReason: reason,
    } as any);
    const phone = agent.phone || agent.user?.phone;
    if (phone) {
      await this.smsService.sendSms(phone,
  ...
  async suspend(agentId: number) {
    ...
    if (!agent) throw new NotFoundException('Wakala hapatikani');
    await this.agentRepo.update(agentId, { status: AgentStatus.SUSPENDED } as any);
    const phone = agent.phone || agent.user?.phone;
    if (phone) {
      await this.smsService.sendSms(phone,
  ...
  async updateSettings(agentId: number, dto: {
    commissionRate?: number; deliveryCommission?: number;
    collectionFeeUrban?: number; collectionFeeRural?: number;
    tier?: string; city?: string;
  }) {
    const agent = await this.agentRepo.findOne({ where: { id: agentId } });
    if (!agent) throw new NotFoundException('Wakala hapatikani');
    const updates: any = {};
    if (dto.commissionRate     != null) updates.commissionRate     = dto.commissionRate;
    if (dto.deliveryCommission != null) updates.deliveryCommission = dto.deliveryCommission;
    if (dto.collectionFeeUrban != null) updates.collectionFeeUrban = dto.collectionFeeUrban;
    if (dto.collectionFeeRural != null) updates.collectionFeeRural = dto.collectionFeeRural;
  ...
  async getAgentsPendingEarnings() {
    ...
    return agents
      ...
      .map(a => ({
        ...
        agentCode: a.agentCode, tier: a.tier,
        pendingEarnings:          Number(a.pendingEarnings),
        totalEarnings:            Number(a.totalEarnings),
  ...
  async releaseEarnings(agentId: number, amount: number, note?: string) {
    ...
    await this.agentRepo.update(agentId, { pendingEarnings: pending - release } as any);
    const phone = agent.phone || agent.user?.phone;
    if (phone) {
      await this.smsService.sendSms(phone,
    ...
    return { message: 'Malipo yametolewa', agentId, released: release, remaining: pending - release };
  }
  ...
  async toggleOnline(user: User) {
    const agent = await this.agentRepo.findOne({ where: { user: { id: user.id } } });
    if (!agent) throw new NotFoundException('Wakala hapatikani');
    if ((agent as any).status !== 'approved') throw new BadRequestException('Akaunti haijaidhinishwa bado');
    ...
    return {
      isOnline: nowOnline,
      message:  nowOnline ? 'Uko mtandaoni — utaona kazi zinazopatikana' : 'Umejisajili nje ya mtandao',
    };
  }
  ...
  async getAvailableAgents(city: string, weightKg?: number) {
    const agents = await this.agentRepo.find({
      where: { status: 'approved' as any } as any,
      relations: { user: true },
    ...
    return agents
      .filter(a => {
        const agentCity = a.city?.toLowerCase() || a.district?.toLowerCase() || '';
      ...
      .map(a => ({
        ...
        agentCode:          a.agentCode,
        agentType:          (a as any).agentType || 'boda',
        agentTypeLabel:     AGENT_TYPE_LABELS[(a as any).agentType || 'boda'] || '🏍️ Boda',
        deliveryTime:       DELIVERY_TIME[(a as any).agentType || 'boda'] || 'Masaa 1-4',
        maxWeightKg:        Number((a as any).maxWeightKg || 20),
        vehicleDescription: (a as any).vehicleDescription || null,
        deliveryFee:        Number(a.deliveryCommission || 1000),
  ...
  async updateProfile(user: User, dto: {
    agentType?:          string;
    maxWeightKg?:        number;
    vehicleDescription?: string;
    ...
    if (dto.vehicleDescription) updates.vehicleDescription = dto.vehicleDescription;
    if (dto.deliveryCommission) updates.deliveryCommission = dto.deliveryCommission;
    if (dto.collectionFeeUrban) updates.collectionFeeUrban = dto.collectionFeeUrban;
    if (dto.collectionFeeRural) updates.collectionFeeRural = dto.collectionFeeRural;
