...
export class OffersService {
  ...
  async makeOffer(buyerId: number, dto: {
    classifiedId: number;
    offerAmount:  number;
    buyerNote?:   string;
  }): Promise<Offer> {
    ...
    if ((classified as any).sellerId === buyerId || classified.seller?.id === buyerId) {
      throw new BadRequestException('Huwezi kutoa bei kwenye bidhaa yako mwenyewe');
    }
    if (dto.offerAmount <= 0) throw new BadRequestException('Bei lazima iwe zaidi ya sifuri');

    // Check existing pending offer
    const existing = await this.offerRepo.findOne({
      where: { classifiedId: dto.classifiedId, buyerId, status: OfferStatus.PENDING }
    });
    if (existing) throw new BadRequestException('Una bei inayosubiri tayari kwa bidhaa hii');

    const expiresAt = new Date();
    expiresAt.setHours(expiresAt.getHours() + 48); // 48hr expiry

    return this.offerRepo.save(this.offerRepo.create({
  ...
  async respondToOffer(sellerId: number, offerId: number, dto: {
  ...
  async getOffersForClassified(classifiedId: number, sellerId: number): Promise<Offer[]> {
    return this.offerRepo.find({
