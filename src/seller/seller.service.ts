...
export class SellerService {
  ...
  async findAllApproved(): Promise<SellerProfile[]> {
    return this.profileRepo.find({
      where: [{ status: SellerStatus.APPROVED }, { status: SellerStatus.PENDING }],
  ...
  async findPublicSellers(viewerId?: number) {
    ...
    if (viewerId) {
      const rows: { sellerId: number }[] = await this.userRepo.query(
        'SELECT "sellerId" FROM follow WHERE "followerId" = $1', [viewerId]
      ).catch(() => []);
      followedIds = new Set(rows.map(r => r.sellerId));
    }

    return profiles
      .filter(p => p.user) // guard against orphaned profiles
      .map(p => ({
        id:               p.user.id, // ✅ USER id — matches /stores/:sellerId route
        businessName:     p.businessName,
        businessCategory: (p as any).businessCategory || null,
        storeName:        p.user.storeName || null,
        storeTagline:     p.user.storeTagline || null,
        storeDescription: p.user.storeDescription || null,
        logo:             p.user.logo || null,
        coverImage:       p.user.coverImage || null,
        rating:           p.user.rating || null,
        isOfficialStore:  p.user.isOfficialStore || false,
        isVerified:       p.user.isVerified || false,
        followersCount:   p.user.followersCount || 0,
        city:             (p as any).businessCity || p.user.businessLocation || null,
        district:         (p as any).businessDistrict || null,
        region:           (p as any).businessRegion || null,
        completedOrders:  p.user.completedOrders  || 0,
        reviewsCount:     p.user.reviewsCount     || 0,
        reputationScore:  (p.user as any).reputationScore || 0,
        userId:           p.user.id,
        createdAt:        p.user.createdAt,
        isFollowing:      followedIds.has(p.user.id),
      }));
  }
  ...
  async findByUserId(userId: number): Promise<any> {
    ...
    const sellerProfile = await this.profileRepo.findOne({
    ...
    return {
      ...
      phone:             u.phone || null,
      businessLocation:  u.businessLocation || (sellerProfile as any)?.businessCity || null,
  ...
  async apply(dto: CreateSellerProfileDto, user: User): Promise<SellerProfile> {
    ...
    if (existing) throw new ConflictException('You already have a seller application');
  ...
  async updateProfile(userId: number, dto: Partial<CreateSellerProfileDto> & {
    phone?: string;
    name?: string;
    email?: string;
...
// Fetch both online orders (via product) and manual shipments (direct seller)
      Promise.all([
        ...
        this.orderRepo
          ...
          .andWhere("o.source IN ('seller_shipment', 'offline', 'offline_intercity')")
          .orderBy('o.createdAt', 'DESC')
          .take(30)
          .getMany(),
        ...
        return [...onlineOrders, ...manualOrders].filter(o => {
    ...
    const pendingOrders = myOrders.filter(o =>
      o.status === OrderStatus.PENDING_PAYMENT || o.status === OrderStatus.PAID
    ).length;
    ...
    const needsShipping = myOrders.filter(o =>
      o.status === OrderStatus.PAID
    ).length;

    // ── Analytics ────────────────────────────────────────────────────────────
    // Per-product revenue
    const productRevenue: Record<number, { name: string; orders: number; revenue: number }> = {};
    for (const order of myOrders) {
      const pid  = order.product?.id || 0;
      const name = order.product?.name || (order as any).manualProductName || 'Bidhaa ya Mkono';
      if (!productRevenue[pid]) productRevenue[pid] = { name, orders: 0, revenue: 0 };
    ...
    const customerSpend: Record<string, { name: string; phone: string; orders: number; spent: number }> = {};
    for (const order of myOrders) {
      const phone = order.buyer?.phone || (order as any).manualBuyerPhone || (order as any).phone || null;
      const name  = order.buyer?.name  || (order as any).manualBuyerName  || 'Mteja';
      if (!phone) continue;
      if (!customerSpend[phone]) customerSpend[phone] = { name, phone, orders: 0, spent: 0 };
    ...
    const repeatCount   = Object.values(customerSpend).filter(c => c.orders > 1).length;
    ...
    for (const order of myOrders) {
      ...
      if (daily[day] !== undefined) daily[day] += Number(order.totalAmount || 0);
    }
    const dailyRevenue = Object.entries(daily).map(([date, revenue]) => ({
      date,
      label: new Date(date).toLocaleDateString('sw-TZ', { weekday: 'short', day: 'numeric' }),
      revenue,
    }));
  ...
  async getMyPayouts(userId: number) {
    ...
    const released  = orders.filter(o => (o as any).payoutStatus === 'released');
    const pending   = orders.filter(o => (o as any).payoutStatus !== 'released'
                                      && (o as any).escrowStatus === 'held');
    const total     = orders.length;

    const totalEarned   = released.reduce((s, o) => s + Number((o as any).sellerAmount || 0), 0);
    const totalPending  = pending.reduce((s, o)  => s + Number((o as any).sellerAmount || 0), 0);
    ...
    return {
      ...
      orders: orders.map(o => ({
        ...
        productName:     (o as any).product?.name || (o as any).manualProductName || 'Bidhaa',
        buyerName:       (o as any).buyer?.name   || (o as any).manualBuyerName   || 'Mteja',
  ...
  async inviteTeamMember(sellerId: number, dto: {
    phone: string;
    role: string;
    permissions: Record<string, boolean>;
  }): Promise<BusinessTeamMember> {
    // Find user by phone
    const user = await this.userRepo.findOne({ where: { phone: dto.phone } as any });
    if (!user) throw new NotFoundException(`Mtumiaji mwenye nambari ${dto.phone} hajapatikana. Waombe ajiandikishe kwanza.`);

    // Don't add self
    if (user.id === sellerId) throw new BadRequestException('Huwezi kujiongeza mwenyewe');

    // Check not already member
    const existing = await this.teamRepo.findOne({ where: { sellerId, userId: user.id, isActive: true } });
    if (existing) throw new BadRequestException('Mtumiaji huyu tayari yuko kwenye timu yako');
  ...
  async updateTeamMember(sellerId: number, memberId: number, dto: {
    role?: string;
    permissions?: Record<string, boolean>;
    isActive?: boolean;
  }): Promise<BusinessTeamMember> {
    const member = await this.teamRepo.findOne({ where: { id: memberId, sellerId } });
