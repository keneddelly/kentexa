...
export class PaymentsService {
  ...
  constructor(
    ...
    @InjectRepository(ClassifiedInvoiceRequest) private classifiedInvoiceRepo: Repository<ClassifiedInvoiceRequest>,
  ...
  private getProvider(provider: string): IPaymentProvider {
    ...
    if (USE_INDIVIDUAL_NETWORKS) {
      switch (provider) {
        case 'vodacom': return this.vodacomService;
        case 'airtel':  return this.airtelService;
        default:        return this.selcomService;
      }
    }
  ...
  async getPayoutSummary(): Promise<{
    ...
    const pendingCount = await this.orderRepo.count({
      where: { paymentStatus: 'paid' as any, escrowStatus: 'held' as any } as any,
    });
  ...
  private async recordAgentCommission(
    agentId: number, amount: number, transactionRef: string,
    paymentMethod: string, invoice?: Invoice | null, orderId?: number | null,
  ): Promise<void> {
    ...
    await this.agentTransactionRepo.save(
      this.agentTransactionRepo.create({
        agent,
        ...(invoice ? { invoice } : {}),
        invoiceAmount: amount, commissionRate, commissionAmount: commission,
        transactionReference: transactionRef, paymentMethod,
  ...
  async initiatePayment(dto: InitiatePaymentDto, user: User) {
    ...
    if (order.paymentStatus === OrderPaymentStatus.PAID) throw new BadRequestException('Order already paid');

    const provider  = this.getProvider(dto.provider);
    const reference = `KNT-ORD-${order.id}-${Date.now()}`;
    const response  = await provider.initiatePayment({
      phone: dto.phone, amount: Number(order.totalAmount), reference,
      description: `Payment for Order #${order.id} on Kentexa`,
    });

    const payment = this.paymentRepo.create({
      order, user, phone: dto.phone, amount: order.totalAmount,
  ...
  async handleCallback(body: any, providerName: string) {
    ...
    const payment = await this.paymentRepo.findOne({ where: { providerRequestId: result.providerRequestId } });
    if (!payment) { this.logger.warn(`No payment found for requestId: ${result.providerRequestId}`); return { message: 'OK' }; }
  ...
  async customerPayInvoice(invoiceNumber: string | undefined, phone: string, provider: string, user: User, orderId?: number) {
    ...
    const response        = await providerService.initiatePayment({
      phone, amount: found.amount, reference,
      description: `Kentexa payment for invoice ${invoiceNumber}`,
    });
    if (!response.success) throw new BadRequestException(response.message);

    const meta = JSON.stringify({
      invoiceType: found.invoiceType, invoiceNumber: found.invoiceNumber,
      invoiceId: found.invoiceId, orderId: found.orderId,
    });

    const payment = this.paymentRepo.create({
      user, amount: found.amount, provider: provider as PaymentProvider,
      status: PaymentStatus.PENDING, providerRequestId: response.providerRequestId, phone, metadata: meta,
    });
    await this.paymentRepo.save(payment);
    ...
    return { success: true, message: response.message, providerRequestId: response.providerRequestId, invoiceNumber, amount: found.amount, phone, provider };
  }
  ...
  async agentInitiatePayment(invoiceNumber: string, agentPhone: string, provider: string, agentUser: User) {
    ...
    const providerService = this.getProvider(provider);
    const response        = await providerService.initiatePayment({
      phone: agentPhone, amount: found.amount, reference,
    ...
    const meta = JSON.stringify({
      invoiceType: found.invoiceType, invoiceNumber: found.invoiceNumber,
      invoiceId: found.invoiceId, orderId: found.orderId, agentId: agent.id,
    });

    const payment = this.paymentRepo.create({
      amount: found.amount, provider: provider as PaymentProvider,
      status: PaymentStatus.PENDING, providerRequestId: response.providerRequestId,
      phone: agentPhone, metadata: meta,
    });
    await this.paymentRepo.save(payment);
    ...
    return { success: true, message: response.message, providerRequestId: response.providerRequestId, invoiceNumber, amount: found.amount, agentPhone, provider };
  }
  ...
  async agentPaymentCallback(body: any, providerName: string) {
    ...
    let meta: any = {};
    try { meta = JSON.parse((payment as any).metadata || '{}'); } catch {}
    ...
    if (!result.success) {
      ...
      if (invoiceType === 'order' && orderId) {
        await this.invoiceRepo.update({ order: { id: orderId } }, { status: InvoiceStatus.AWAITING_PAYMENT, agentId: null });
      }
      return { message: 'OK' };
    }
    ...
    if (invoiceType === 'order' && orderId) {
      ...
      orderInvoice = await this.invoiceRepo.findOne({ where: { order: { id: orderId } } });
      if (orderInvoice) {
    ...
    } else if ((invoiceType === 'classified' || invoiceType === 'manual') && invoiceNumber) {
      await this.classifiedInvoiceRepo.update(
        { invoiceNumber },
        { status: ClassifiedInvoiceStatus.PAID, paidAt: new Date(), transactionReference: transactionRef, paymentMethod: providerName },
      );
    ...
    if (agentId) {
      await this.recordAgentCommission(agentId, Number(payment.amount), transactionRef, providerName, orderInvoice, orderId);
    }

    this.logger.log(`Payment confirmed & order auto-confirmed: ${transactionRef} type=${invoiceType}`);
    return { message: 'OK' };
  }

  async mockAgentCallback(providerRequestId: string) {
    return this.agentPaymentCallback(
      { status: 'SUCCESS', providerRequestId, transactionId: `MOCK-TXN-${Date.now()}` },
      'mock',
    );
  }
  ...
  async getAgentDashboard(agentUser: User) {
    ...
    const transactions = await this.agentTransactionRepo.find({
      where: { agent: { id: agent.id } }, order: { createdAt: 'DESC' }, take: 20,
