...
import { IPaymentProvider, PaymentRequest, PaymentResponse, CallbackResult } from '../payment-provider.interface';
...
export class ClickPesaService implements IPaymentProvider {
  ...
  private readonly apiUrl     = process.env.CLICKPESA_API_URL     || 'https://api.clickpesa.com/v1';
  private readonly apiKey     = process.env.CLICKPESA_API_KEY     || 'YOUR_CLICKPESA_API_KEY';
  private readonly merchantId = process.env.CLICKPESA_MERCHANT_ID || 'YOUR_MERCHANT_ID';
  // ─────────────────────────────────────────────────────────────────────────

  async initiatePayment(request: PaymentRequest): Promise<PaymentResponse> {
    this.logger.log(`ClickPesa: initiating payment of ${request.amount} to ${request.phone}`);
    ...
    try {
      const response = await fetch(`${this.apiUrl}/payments/initiate`, {
        ...
        body: JSON.stringify({
          ...
          callback_url: process.env.CLICKPESA_CALLBACK_URL || 'https://yourdomain.com/payments/clickpesa/callback',
        }),
      });
  ...
  parseCallback(body: any): CallbackResult {
    ...
    const success = body.status === 'SUCCESS' || body.payment_status === 'COMPLETED';

    return {
      success,
      providerRequestId: body.request_id   || body.transaction_id || body.providerRequestId,
      providerReference: body.reference_id || body.transaction_id || null,
      failureReason:     success ? null : (body.message || body.failure_reason || 'Payment failed'),
    };
  }
}