...
export class BodaRatesService implements OnModuleInit {
  ...
  private async seedDefaultRates() {
    const defaults: Partial<BodaRateCard>[] = [
      ...
      {
        rateKey: 'zone_1',
        label: 'Eneo 1 — Kariakoo / Ilala / Upanga',
        category: 'zone',
        fee: 500,
        sortOrder: 1,
        isActive: true,
      ...
      {
        rateKey: 'zone_2',
        label: 'Eneo 2 — Kinondoni / Ubungo / Temeke',
        category: 'zone',
        fee: 1500,
        sortOrder: 2,
        isActive: true,
      ...
      {
        rateKey: 'zone_3',
        label: 'Eneo 3 — Mbagala / Mbezi / Kimara',
        category: 'zone',
        fee: 3000,
        sortOrder: 3,
        isActive: true,
      ...
      {
        rateKey: 'zone_4',
        label: 'Eneo 4 — Bunju / Tegeta / Boko',
        category: 'zone',
        fee: 6000,
        sortOrder: 4,
        isActive: true,
  ...
  async calculateFee(
    ...
    const weightRate = weightRates.find(
      (r) => weight >= Number(r.weightMin) && weight < Number(r.weightMax),
    );
    const weightFee = weightRate?.fee || 0;
    if (weightFee > 0) {
      breakdown.push({
        label: weightRate?.label || 'Ada ya Uzito',
        fee: weightFee,
      });
    }
  ...
  async updateRate(
    ...
    if (!rate) {
      ...
      return this.rateRepo.save(
        this.rateRepo.create({ rateKey, ...dto } as any),
      );
    }
