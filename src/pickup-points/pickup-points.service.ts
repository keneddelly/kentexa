...
export class PickupPointsService {
  ...
  async findByCity(city: string): Promise<PickupPoint[]> {
    return this.repo.createQueryBuilder('p')
      ...
      .where('LOWER(p.city) LIKE LOWER(:city)', { city: `%${city.split(',')[0].trim()}%` })
  ...
  // ── Update status ─────────────────────────────────────────────────────────
  async updateStatus(userId: number, id: number, status: PickupPointStatus): Promise<PickupPoint> {
  ...
  // ── Update info ───────────────────────────────────────────────────────────
  async update(userId: number, id: number, dto: Partial<{
    name: string; address: string; landmark: string;
    phone: string; openHours: string; latitude: number; longitude: number;
  }>): Promise<PickupPoint> {
    const pt = await this.repo.findOne({ where: { id, userId } });
    if (!pt) throw new NotFoundException('Mahali pa kupokelea haikupatikana');
