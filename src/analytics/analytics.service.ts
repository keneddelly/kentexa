...
// ua-parser-js v2 uses named export
...
export class AnalyticsService {
  ...
  constructor(
    @InjectRepository(AnalyticsSession) private sessionRepo: Repository<AnalyticsSession>,
  ...
  async trackEvent(dto: {
    ...
    try {
      ...
      if (!session) {
        ...
        try {
          ...
          if (dupErr.code === '23505') {
            session = await this.sessionRepo.findOne({ where: { sessionId: dto.sessionId } });
      ...
      await this.sessionRepo.update(session.id, updates as any);
  ...
  async getDashboardStats(days = 7) {
    ...
    const [
      ...
      this.eventRepo.count({ where: { eventType: 'page_view', createdAt: MoreThanOrEqual(since) } }),
      this.sessionRepo.count({ where: { converted: true, createdAt: MoreThanOrEqual(since) } }),
      // Sessions active in last 5 minutes
      this.sessionRepo.count({ where: { lastSeenAt: MoreThanOrEqual(new Date(Date.now() - 5 * 60 * 1000)) } }),
    ]);
    ...
    const topPages = await this.eventRepo
      ...
      .where('e.eventType = :type AND e.createdAt >= :since', { type: 'page_view', since })
      .groupBy('e.page')
      .orderBy('views', 'DESC')
      .limit(10)
      .getRawMany();
    ...
    const topProducts = await this.eventRepo
      ...
      .addSelect('COUNT(*)', 'views')
      .where('e.eventType = :type AND e.createdAt >= :since', { type: 'product_view', since })
    ...
    const topSearches = await this.eventRepo
      ...
      .where('e.eventType = :type AND e.createdAt >= :since', { type: 'search', since })
    ...
    const sources = await this.sessionRepo
      .createQueryBuilder('s')
      .select('COALESCE(s.utmSource, CASE WHEN s.referrer IS NULL THEN \'Direct\' WHEN s.referrer LIKE \'%google%\' THEN \'Google\' WHEN s.referrer LIKE \'%whatsapp%\' THEN \'WhatsApp\' WHEN s.referrer LIKE \'%facebook%\' THEN \'Facebook\' ELSE \'Other\' END)', 'source')
      .addSelect('COUNT(*)', 'count')
      .where('s.createdAt >= :since', { since })
      .groupBy('1')
      .orderBy('count', 'DESC')
    ...
    return {
      summary: {
        ...
        avgEventsPerSession: totalSessions > 0 ? (totalEvents / totalSessions).toFixed(1) : '0',
      },
      topPages,
      topProducts,
      topSearches,
