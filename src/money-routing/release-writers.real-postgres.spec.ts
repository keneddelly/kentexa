...
describe('I2G release-writer inventory — every seller-release path is closed through the canonical operation', () => {
  ...
  it('InvoicesService.markPaid (digital product): canonical release; blocked -> paid-but-held', async () => {
    ...
    // PaymentConfirmationService's own confirmation logic (currency/reference/entry-state/allow-list
    // checks) is exhaustively covered by its own real-Postgres spec -- this test's job is only the
    // release-WRITER boundary (does markPaid's digital-order branch route through the canonical
    // OrderReleaseService), so it's stubbed here to return the same shape a real CONFIRMED/
    // DIGITAL_COMPLETED outcome would, without replicating the confirmation service's own SQL
    // machinery against this disposable schema.
    const mk = (orderId: number) => {
      const invoice: any = { id: 1, invoiceNumber: `INV-${orderId}`, status: 'awaiting_payment', amount: 1000, order: { id: orderId, product: { productType: 'digital' }, seller: { id: w.owner.id }, sellerAmount: 1000 }, buyer: { id: 3 } };
      const svc: any = build(InvoicesService, {
        orderRelease: h.release,
        findByInvoiceNumber: async () => invoice,
        generateReceiptNumber: async () => 'R-1',
        invoiceRepo: { save: async (i: any) => i, update: async () => undefined },
        paymentRepo: { create: (x: any) => x, save: async (x: any) => ({ ...x, id: 1 }) },
        paymentConfirmation: { confirmVerifiedPayment: async () => ({ status: 'CONFIRMED', paymentId: 1, orderId, classifiedInvoiceNumber: null, orderTransition: 'DIGITAL_COMPLETED', classifiedTransitioned: false }) },
        activityEvents: { record: () => undefined },
        orderRepo: orderRepoStub(),
        logger: { log: () => {}, warn: () => {} },
      });
      return svc;
    };
    await mk(w.good).markPaid(`INV-${w.good}`, 'TX1', 'harness manual confirm', 1);
    await assertReleased(w, w.good, 1000);
    await mk(w.blocked).markPaid(`INV-${w.blocked}`, 'TX2', 'harness manual confirm', 1); // does not throw: the payment succeeded
    await assertHeld(w.blocked);
