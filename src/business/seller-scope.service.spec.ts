...
describe('SellerScopeService', () => {
  ...
  describe('resolveScope() — Business-First Stage 2A', () => {
    ...
    it('VALID ORGANIZATIONAL SELLER: returns workspaceId from an already-resolved RoleContext, mode="workspace"', async () => {
      ...
      expect(scope).toEqual({ legacySellerId: 1, workspaceId: 2, businessId: null, mode: 'workspace', profileType: null, profileId: null, commerceProfileId: null, identityType: null, delegated: false });
    });
    ...
    it('VALID unresolved legacy Seller: a real RoleContext with workspaceId=null is legacy mode, not a failure', async () => {
      ...
      const scope = await service.resolveScope(1, userWithPayload(1, AccountRoleType.SELLER), roleContext);
      expect(scope).toEqual({ legacySellerId: 1, workspaceId: null, businessId: null, mode: 'legacy', profileType: null, profileId: null, commerceProfileId: null, identityType: null, delegated: false });
    });
    ...
    it('resolves fresh from the payload when no RoleContext is supplied, and still returns workspaceId from it', async () => {
      ...
      expect(scope).toEqual({ legacySellerId: 1, workspaceId: 5, businessId: null, mode: 'workspace', profileType: null, profileId: null, commerceProfileId: null, identityType: null, delegated: false });
