...
export class PriceSuggestionService {
  ...
  async suggest(params: {
    ...
    if (params.condition) {
      qb.andWhere('LOWER(c.condition) = LOWER(:cond)', {
        cond: params.condition,
      });
    }
    if (params.subcategory) {
      qb.andWhere('LOWER(c.subcategory) = LOWER(:sub)', {
    ...
    if (prices.length === 0) {
      return {
        ...
        currency: 'TZS',
        insights: [
          'Hakuna data ya bei kwa kategoria hii bado. Weka bei unayoona inafaa.',
        ],
        confidence: 'low',
      };
    }
    ...
    insights.push(
      `Bidhaa kama hii zinauzwa kati ya TZS ${fmt(low)} – TZS ${fmt(high)}.`,
    );

    if (params.condition === 'new') {
      insights.push(
        'Bidhaa mpya kawaida zinauzwa kwa bei ya juu kuliko wastani.',
      );
    } else if (params.condition === 'used') {
      insights.push(
        'Bidhaa zilizotumika zinauzwa vizuri bei chini ya wastani wa soko.',
      );
    }

    if (prices.length >= 20) {
      insights.push(
        `Takwimu zinatoka kwenye matangazo ${prices.length} yenye kufanana.`,
      );
    }
  ...
  async checkPrice(classifiedId: number): Promise<{
    ...
    const listing = await this.classifiedRepo.findOne({
      where: { id: classifiedId },
    });
    if (!listing || !listing.price)
      return { status: 'unknown', message: 'Tangazo halijapatikana' };
    ...
    if (suggestion.sampleSize < 3) {
      return {
        status: 'unknown',
        message: 'Data haitoshi kufanya ulinganisho',
      };
    }
