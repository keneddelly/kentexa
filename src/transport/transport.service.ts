...
import { TransportProvider, ProviderStatus, ConfirmMode } from './entities/transport-provider.entity';
...
import { TransportAssignment, AssignmentStatus }          from './entities/transport-assignment.entity';
...
export class TransportService {
  constructor(
    @InjectRepository(TransportProvider)   private providerRepo:    Repository<TransportProvider>,
  ...
  async register(user: User, dto: {
    name: string; type: string; contactPhone: string;
    whatsappPhone?: string; contactEmail?: string;
    registrationNumber?: string; description?: string;
    defaultParcelCapacity?: number; defaultMaxWeightKg?: number;
  }): Promise<TransportProvider> {
    // One provider per user
    const existing = await this.providerRepo.findOne({ where: { userId: user.id } });
    if (existing) throw new BadRequestException('Una akaunti ya usafirishaji tayari');
    ...
    const provider = this.providerRepo.create({
      ...
      confirmMode:            ['bus', 'courier'].includes(dto.type) ? ConfirmMode.AUTO : ConfirmMode.MANUAL,
    });
  ...
  async findPublicByUserId(userId: number) {
    ...
    if (!p || ![ProviderStatus.VERIFIED, ProviderStatus.ACTIVE].includes(p.status)) return null;
  ...
  async updateProfile(userId: number, dto: Partial<TransportProvider>): Promise<TransportProvider> {
    const p = await this.getMyProfile(userId);
    const allowed = ['name','contactPhone','whatsappPhone','contactEmail',
  ...
  async addRoute(userId: number, dto: {
    routeType: string;
    originCity?: string; destinationCity?: string; transitCities?: string[];
    loopStops?: string[];
    coverageWards?: string[]; coverageCity?: string;
    pricePerKg?: number; fixedFee?: number;
    ...
    const route = await this.routeRepo.save(this.routeRepo.create({
    ...
    try {
      ...
      if (city) {
        const ad = await this.serviceAdRepo.findOne({
          where: { providerId: provider.userId ?? undefined, category: ServiceCategory.USAFIRISHAJI },
        });
        if (ad) {
  ...
  async updateRoute(userId: number, routeId: number, dto: any): Promise<TransportRoute> {
    const p = await this.getMyProfile(userId);
  ...
  async publishAvailability(userId: number, dto: {
    routeId?: number;
    date: string; departureTime?: string; arrivalEstimate?: string;
    bookingDeadline?: string;
    totalSlots: number; totalCapacityKg?: number;
    fromCity?: string; toCity?: string; notes?: string;
    ...
    if (dto.routeId) {
      ...
      if (dup) throw new BadRequestException('Umeshaweka upatikanaji kwa tarehe hii');
    }
    return this.availabilityRepo.save(this.availabilityRepo.create({
      providerId:       p.id,
  ...
  async getMyAvailability(userId: number, days = 7): Promise<ProviderAvailability[]> {
    const p     = await this.getMyProfile(userId);
  ...
  async updateAvailabilityStatus(userId: number, availId: number, status: AvailabilityStatus): Promise<ProviderAvailability> {
    const p = await this.getMyProfile(userId);
    const a = await this.availabilityRepo.findOne({ where: { id: availId, providerId: p.id } });
   ...
   */
  async findAvailableForRoute(fromCity: string, toCity: string): Promise<{
    ...
    const providers = await this.providerRepo
      ...
      .innerJoin('p.user', 'u')
      .leftJoin('transport_route', 'r', 'r.providerId = p.id AND r.isActive = true')
  ...
  async createAssignment(superAgent: User, dto: {
    trackingNumber?: string; orderId?: number; parcelId?: number;
    providerId: number; availabilityId?: number;
    fromCity: string; toCity: string;
    parcelCount?: number; weightKg?: number;
    agreedPrice?: number; scheduledDeparture?: string;
    ...
    if (dto.availabilityId) {
      const avail = await this.availabilityRepo.findOne({ where: { id: dto.availabilityId } });
      if (avail && avail.usedSlots < avail.totalSlots) {
        avail.usedSlots++;
        avail.usedCapacityKg += (dto.weightKg || 1);
        if (avail.usedSlots >= avail.totalSlots) avail.status = AvailabilityStatus.FULL;
    ...
    const assignment = await this.assignmentRepo.save(this.assignmentRepo.create({
      ...
      status: isAutoConfirm ? AssignmentStatus.ACCEPTED : AssignmentStatus.PENDING,
  ...
  async respondToAssignment(userId: number, assignmentId: number, accept: boolean, declineReason?: string) {
    const provider = await this.getMyProfile(userId);
    const assignment = await this.assignmentRepo.findOne({
      where: { id: assignmentId, providerId: provider.id, status: AssignmentStatus.PENDING },
    });
  ...
  async updateAssignmentStatus(userId: number, assignmentId: number, dto: {
    ...
    switch (dto.status) {
      ...
      case AssignmentStatus.COMPLETED:
        ...
        if (provider.userId) {
          this.reputationService.award(
            provider.userId,
            ReputationEventType.TRANSPORT_COMPLETED,
            { sourceEntityType: 'transport_assignment', sourceEntityId: a.id }
          ).catch(() => {});
        }
        break;
    }
  ...
  async getMyAssignments(userId: number, status?: string): Promise<TransportAssignment[]> {
    const p = await this.getMyProfile(userId);
    const q = this.assignmentRepo.createQueryBuilder('a')
  ...
  async getAssignmentByTracking(trackingNumber: string): Promise<TransportAssignment[]> {
  ...
  private async syncServiceAd(provider: TransportProvider, activate: boolean): Promise<void> {
    try {
      // Find existing linked service ad
      let ad = await this.serviceAdRepo.findOne({
        where: { providerId: provider.userId ?? undefined, category: ServiceCategory.USAFIRISHAJI },
      });
      ...
      if (ad) {
        ...
        // Create new
        await this.serviceAdRepo.save(this.serviceAdRepo.create({
      ...
      console.warn('Failed to sync transport provider to service marketplace:', err?.message);
    }
  }

  async adminVerify(id: number, approve: boolean, reason?: string): Promise<TransportProvider> {
  ...
  async adminGetAssignments(filters: { providerId?: number; status?: string; fromCity?: string; toCity?: string }): Promise<TransportAssignment[]> {
    const q = this.assignmentRepo.createQueryBuilder('a')
      .leftJoinAndSelect('a.provider', 'p')
      .leftJoinAndSelect('a.assignedBy', 'sa');
    if (filters.providerId) q.andWhere('a.providerId = :pid', { pid: filters.providerId });
    if (filters.status)     q.andWhere('a.status = :st', { st: filters.status });
    if (filters.fromCity)   q.andWhere('LOWER(a.fromCity) LIKE LOWER(:fc)', { fc: `%${filters.fromCity}%` });
    if (filters.toCity)     q.andWhere('LOWER(a.toCity) LIKE LOWER(:tc)', { tc: `%${filters.toCity}%` });
