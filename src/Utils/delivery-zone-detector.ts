...
export const DAR_KEYWORDS = [
  ...
  'Manzese', 'Tandale', 'Magomeni', 'Mwananyamala', 'Kigogo',
  // Ubungo
  'Ubungo', 'Kimara', 'Mbezi', 'Mbezi Louis', 'Mbezi Beach',
  'Makuburi', 'Kibamba',
  // Temeke / South
  'Temeke', 'Mbagala', 'Mbagala Kuu', 'Kigamboni', 'Chang\'ombe',
  'Mtoni', 'Kurasini', 'Tandika', 'Yombo',
  // North / Far
  'Bunju', 'Bunju A', 'Bunju B', 'Mbweni', 'Tegeta', 'Boko',
  'Kunduchi', 'Msasani', 'Masaki', 'Oyster Bay',
  // General
  'Dar es Salaam', 'Dar', 'DSM',
];
...
export const BATCH_ZONES: Record<string, string[]> = {
  ...
  Kinondoni:['Kinondoni', 'Mwenge', 'Mikocheni', 'Sinza', 'Kijitonyama', 'Msasani', 'Masaki', 'Oyster Bay'],
};
...
export function getAvailableDeliveryMethods(
  ...
  methods: Array<{ key: string; label: string; icon: string; fee: number | null }>;
} {
  const buyerInDar   = isDarEsSalaam(buyerAddress);
  const sellerInDar  = !sellerCity || isDarEsSalaam(sellerCity) || sellerCity?.toLowerCase().includes('dar');
  ...
  if (!isSameCity) {
    return {
      isSameCity: false,
      hasBatchZone: false,
      methods: [{ key: 'agent', label: 'KenteXa Super Agent', icon: '🏢', fee: null }],
    };
  }

  const methods: Array<{ key: string; label: string; icon: string; fee: number | null }> = [
    { key: 'boda', label: 'Boda Boda', icon: '🛵', fee: null }, // fee comes from product.bodaFee
  ];

  if (hasBatchZone) {
    methods.push({ key: 'kentexa_delivery', label: 'KenteXa Delivery', icon: '🚐', fee: 3000 });
  }
