...
export class InvoicesService {
  ...
  constructor(
    ...
    @InjectRepository(ReceiptCounter) private receiptCounterRepo: Repository<ReceiptCounter>,
  ...
  async generateInvoiceNumber(): Promise<string> {
    ...
    return await this.dataSource.transaction(async manager => {
      ...
      if (!counter) counter = manager.create(InvoiceCounter, { year, lastSequence: 0 });
  ...
  async generateReceiptNumber(): Promise<string> {
    ...
    return await this.dataSource.transaction(async manager => {
      ...
      if (!counter) counter = manager.create(ReceiptCounter, { year, lastSequence: 0 });
      counter.lastSequence += 1;
  ...
  async createForOrder(order: Order): Promise<Invoice> {
    ...
    const invoice = this.invoiceRepo.create({
      invoiceNumber, order, buyer: order.buyer || null,
      amount: order.totalAmount, status: InvoiceStatus.AWAITING_PAYMENT, dueDate,
    } as any);
  ...
  async findMyInvoices(userId: number): Promise<Invoice[]> {
    return this.invoiceRepo.find({ where: { buyer: { id: userId } }, order: { createdAt: 'DESC' } });
  }
  ...
  async markPaymentProcessing(invoiceNumber: string, paymentMethod: string, agentId?: number): Promise<Invoice> {
    const invoice = await this.findByInvoiceNumber(invoiceNumber);
    if (invoice.status !== InvoiceStatus.AWAITING_PAYMENT) {
      throw new BadRequestException(`Invoice cannot be processed. Current status: ${invoice.status}`);
    }
  ...
  async markPaid(invoiceNumber: string, transactionReference: string): Promise<Invoice> {
    const invoice = await this.findByInvoiceNumber(invoiceNumber);

    if (invoice.status === InvoiceStatus.PAID) throw new BadRequestException('Invoice already paid');
    if (![InvoiceStatus.PAYMENT_PROCESSING, InvoiceStatus.AWAITING_PAYMENT].includes(invoice.status)) {
      throw new BadRequestException(`Invoice cannot be marked paid. Current status: ${invoice.status}`);
    }
    ...
    this.logger.log(`Invoice ${invoiceNumber} PAID & Order auto-confirmed. Receipt: ${receiptNumber}`);
    return saved;
  }
  ...
  async verifyReceipt(receiptNumber: string) {
    const invoice = await this.invoiceRepo.findOne({ where: { receiptNumber } });
    if (!invoice || invoice.status !== InvoiceStatus.PAID) {
      return { valid: false, message: 'Receipt not found or invalid' };
    }
    return {
      valid: true, receiptNumber: invoice.receiptNumber,
      invoiceNumber: invoice.invoiceNumber, amount: invoice.amount,
      paidAt: invoice.paidAt, paymentMethod: invoice.paymentMethod,
  ...
  async generateInvoicePDF(invoiceNumber: string): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      doc.fillColor('#1d4ed8').fontSize(28).font('Helvetica-Bold').text('KenteXa', 50, 35);
      doc.fillColor('#94a3b8').fontSize(11).font('Helvetica').text('Marketplace Tanzania', 50, 70);
      doc.fillColor('#ffffff').fontSize(11)
        .text('kentexa.com', 400, 50, { align: 'right' })
        .text('support@kentexa.com', 400, 68, { align: 'right' });

      doc.fillColor('#0f172a').fontSize(22).font('Helvetica-Bold').text('INVOICE', 50, 145);
      doc.fillColor('#1d4ed8').fontSize(13).font('Helvetica').text(invoice.invoiceNumber, 50, 172);

      const statusColors: Record<string, string> = {
        paid: '#16a34a', awaiting_payment: '#ca8a04',
        expired: '#dc2626', cancelled: '#dc2626', payment_processing: '#2563eb',
      };
      doc.fillColor(statusColors[invoice.status] || '#64748b').fontSize(12).font('Helvetica-Bold')
        .text(`STATUS: ${invoice.status.toUpperCase().replace(/_/g, ' ')}`, 400, 145, { align: 'right' });

      doc.fillColor('#64748b').fontSize(10)
        .text(`Date: ${new Date(invoice.createdAt).toLocaleDateString('en-GB')}`, 400, 162, { align: 'right' })
        .text(`Due: ${invoice.dueDate ? new Date(invoice.dueDate).toLocaleDateString('en-GB') : 'N/A'}`, 400, 179, { align: 'right' });

      doc.strokeColor('#e2e8f0').lineWidth(1).moveTo(50, 205).lineTo(550, 205).stroke();

      doc.fillColor('#64748b').fontSize(10).font('Helvetica-Bold').text('BILL TO:', 50, 225);
      ...
      doc.fillColor('#ffffff').fontSize(10).font('Helvetica-Bold')
        .text('PRODUCT', 60, 320).text('QTY', 350, 320)
        .text('UNIT PRICE', 400, 320).text('TOTAL', 490, 320);
      ...
      doc.fillColor('#0f172a').fontSize(10).font('Helvetica')
        ...
        .text(`TZS ${Number(invoice.order?.product?.displayPrice || invoice.order?.product?.basePrice || 0).toLocaleString()}`, 390, 352)
        .text(`TZS ${Number(invoice.amount).toLocaleString()}`, 470, 352);

      doc.strokeColor('#e2e8f0').lineWidth(1).moveTo(50, 385).lineTo(550, 385).stroke();
      ...
      if (invoice.status === InvoiceStatus.PAID) {
        ...
        doc.fillColor('#16a34a').fontSize(12).font('Helvetica-Bold')
          .text('✓ PAID', 70, 472).text(`Receipt: ${invoice.receiptNumber}`, 70, 488);
        doc.fillColor('#16a34a').fontSize(10).font('Helvetica')
          .text(`Paid on: ${invoice.paidAt ? new Date(invoice.paidAt).toLocaleString() : 'N/A'}`, 300, 475)
          .text(`Ref: ${invoice.transactionReference || 'N/A'}`, 300, 492);
      }

      doc.fillColor('#0f172a').rect(0, doc.page.height - 80, doc.page.width, 80).fill();
      doc.fillColor('#94a3b8').fontSize(10).font('Helvetica')
        .text('Thank you for choosing KenteXa Marketplace!', 50, doc.page.height - 55, { align: 'center' })
        .text('kentexa.com | support@kentexa.com', 50, doc.page.height - 38, { align: 'center' });
  ...
  async generateReceiptPDF(receiptNumber: string): Promise<Buffer> {
    const invoice = await this.invoiceRepo.findOne({ where: { receiptNumber } });
    if (!invoice || invoice.status !== InvoiceStatus.PAID) throw new NotFoundException('Receipt not found');
    ...
    return new Promise((resolve, reject) => {
      ...
      doc.fillColor('#10b981').fontSize(24).font('Helvetica-Bold').text('✓ RECEIPT', 50, 30);
      doc.fillColor('#94a3b8').fontSize(10).font('Helvetica').text('KENTEXA MARKETPLACE', 50, 62);
      doc.fillColor('#ffffff').fontSize(10).text('kentexa.com', 0, 50, { align: 'right', width: doc.page.width - 30 });

      doc.fillColor('#0f172a').fontSize(16).font('Helvetica-Bold').text(receiptNumber, 50, 120);
      doc.fillColor('#64748b').fontSize(10).font('Helvetica')
        .text(`Invoice: ${invoice.invoiceNumber}`, 50, 142)
        .text(`Date: ${invoice.paidAt ? new Date(invoice.paidAt).toLocaleString() : 'N/A'}`, 50, 158);

      doc.strokeColor('#e2e8f0').lineWidth(1).moveTo(50, 180).lineTo(doc.page.width - 50, 180).stroke();
      ...
      details.forEach(item => {
        doc.fillColor('#64748b').fontSize(10).text(item.label + ':', 50, y);
        doc.fillColor('#0f172a').fontSize(10).font('Helvetica-Bold').text(item.value, 200, y);
        doc.font('Helvetica');
        y += 20;
      });

      doc.strokeColor('#e2e8f0').lineWidth(1).moveTo(50, y + 5).lineTo(doc.page.width - 50, y + 5).stroke();
      ...
      doc.fillColor('#f0fdf4').rect(50, y + 55, doc.page.width - 100, 40).fill();
      doc.fillColor('#16a34a').fontSize(11).font('Helvetica-Bold')
        .text('Payment verified by KenteXa Payment System', 60, y + 68, { align: 'center', width: doc.page.width - 120 });
