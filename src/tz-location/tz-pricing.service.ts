...
export class TzPricingService {
  ...
  private districtMultiplier(
    district: TzDistrict | null,
    ...
    if (route) {
      ...
      if (district.isUrban)
        return Number(route.districtMultiplierUrban) || 1.05;
      return Number(route.districtMultiplierRural) || 1.1;
    }
  ...
  async estimate(params: {
    ...
    if (earlyCapital !== params.destinationCity) {
      const capitalRoute = await this.routeRepo
        ...
        .where('LOWER(r.originCity) LIKE LOWER(:origin)', {
          origin: `%${params.originCity}%`,
        })
        .andWhere('LOWER(r.destinationCity) LIKE LOWER(:cap)', {
          cap: `%${earlyCapital}%`,
        })
    ...
    const originCapital =
      this.REGION_CAPITALS[originRegionName] || originRegionName;
    ...
    if (regionRoute) {
      ...
      const isDifferent =
        params.destinationCity !== regionRoute.destinationCity;
      ...
      return {
        ...
        transitCity:
          regionRoute.transitCity ||
          (isDifferent ? regionRoute.destinationCity : null),
    ...
    if (anyRoute) {
      ...
      const extraKg = Math.max(weight - 1, 0);
      const total = this.round500((base + extraKg * perKg) * 1.1);

      return {
        basePrice: base,
        districtFee: this.round500(base * 0.1),
        totalPrice: total,
        perKgFee: perKg,
