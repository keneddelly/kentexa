...
interface RegionSeed {
  name: string; nameSw?: string; capital?: string;
  code?: string; lat: number; lng: number; sortOrder?: number;
}

interface DistrictSeed {
  name: string; nameSw?: string;
  lat: number; lng: number; isUrban?: boolean;
}
...
export class TzLocationService {
  ...
  async seedAll() {
    ...
    // 2. Seed districts
    const districtsByRegion = TZ_DISTRICTS_BY_REGION as Record<string, DistrictSeed[]>;
    for (const [regionName, districts] of Object.entries(districtsByRegion)) {
      const region = await this.regionRepo.findOne({ where: { name: regionName } });
      if (!region) continue;
      for (const d of districts) {
        ...
        if (!exists) {
          await this.districtRepo.save(this.districtRepo.create({
            ...d, region, regionId: region.id,
          } as any));
          results.districts++;
        }
      }
    ...
    const otherWards = OTHER_CITY_WARDS as Record<string, Record<string, WardSeed[]>>;
  ...
  async getDistrictsByRegionName(regionName: string) {
    ...
    if (!region) throw new NotFoundException(`Mkoa haukupatikana: ${regionName}`);
  ...
  async getWardsByDistrictName(districtName: string, regionName?: string) {
    ...
    if (!district) throw new NotFoundException(`Wilaya haikupatikana: ${districtName}`);
  ...
  async resolveDeliveryType(
    ...
    type: 'same_ward' | 'same_district' | 'same_region' | 'intercity' | 'unknown';
    estimatedMinutes?: number;
    suggestedMethod: 'boda' | 'car' | 'van' | 'bus';
  }> {
    if (!originWardId || !destWardId) {
      if (originRegionName && destRegionName) {
        const same = originRegionName.toLowerCase() === destRegionName.toLowerCase();
        return { type: same ? 'same_region' : 'intercity', suggestedMethod: same ? 'car' : 'bus' };
      }
      return { type: 'unknown', suggestedMethod: 'bus' };
    }

    if (originWardId === destWardId) {
      return { type: 'same_ward', estimatedMinutes: 20, suggestedMethod: 'boda' };
    }
    ...
    if (origin.districtId === dest.districtId) {
      return { type: 'same_district', estimatedMinutes: 45, suggestedMethod: 'boda' };
    }
    if (origin.regionId === dest.regionId) {
      return { type: 'same_region', estimatedMinutes: 90, suggestedMethod: 'car' };
    }
