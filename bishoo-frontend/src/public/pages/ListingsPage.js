    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#f1f5f9', fontFamily: "'Inter','Segoe UI',sans-serif" }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#1e1b4b,#1d4ed8)', padding: '16px 16px 14px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          <input type="text" placeholder="Search products and classifieds..."
      ...
      <div style={{ backgroundColor: '#fff', borderBottom: '2px solid #f1f5f9', display: 'flex' }}>
        {[
          ...
          { key: 'classifieds', label: `📋 Ads (${filteredClassifieds.length})` },
        ].map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            style={{ flex: 1, padding: '12px 8px', border: 'none', background: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700, color: tab === t.key ? '#1d4ed8' : '#64748b', borderBottom: tab === t.key ? '3px solid #1d4ed8' : '3px solid transparent', marginBottom: -2, whiteSpace: 'nowrap' }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: '12px 16px 80px', flex: 1, maxWidth: 1100, margin: '0 auto', width: '100%', boxSizing: 'border-box' }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '48px 20px', backgroundColor: '#fff', borderRadius: 16 }}>
            ...
            <p style={{ fontWeight: 700, color: '#1e293b' }}>No results found</p>
            {search && (
              <button onClick={() => { setSearch(''); setSearchInput(''); }}
              ...
              <div style={{ marginBottom: 24 }}>
                {tab === 'all' && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                    ...
                    <span onClick={() => setTab('products')} style={{ fontSize: 12, color: '#1d4ed8', fontWeight: 700, cursor: 'pointer' }}>See all →</span>
                  </div>
                )}
                  ...
                  {filteredProducts.map(p => (
                    <div key={p.id} className="lp-card" onClick={() => onNavigate(`ProductDetail-${p.id}`)}>
                      <div style={{ width: '100%', aspectRatio: '1/1', backgroundColor: '#f1f5f9', overflow: 'hidden', position: 'relative' }}>
                        ...
                        <span style={{ position: 'absolute', top: 6, left: 6, fontSize: 9, fontWeight: 800, padding: '2px 6px', borderRadius: 6, backgroundColor: '#1d4ed8', color: '#fff' }}>🏪 STORE</span>
                        {!p.isAvailable && (
                          <div style={{ position: 'absolute', inset: 0, backgroundColor: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <span style={{ color: '#fff', fontSize: 10, fontWeight: 800 }}>OUT OF STOCK</span>
                          </div>
                        )}
                      </div>
                      <div style={{ padding: '8px 10px' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <span style={{ fontSize: 9, color: '#16a34a', fontWeight: 700 }}>🚚 FREE</span>
              ...
              <div>
                {tab === 'all' && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                    ...
                    <span onClick={() => setTab('classifieds')} style={{ fontSize: 12, color: '#1d4ed8', fontWeight: 700, cursor: 'pointer' }}>See all →</span>
                  </div>
                )}
                <div className="lp-grid">
                  {filteredClassifieds.map(item => (
                    <div key={item.id} className="lp-card" onClick={() => onNavigate(`ClassifiedDetail-${item.id}`)}>
                      <div style={{ position: 'relative' }}>
                        ...
                        <span style={{ position: 'absolute', top: 6, left: 6, fontSize: 9, fontWeight: 800, padding: '2px 6px', borderRadius: 6, backgroundColor: '#db2777', color: '#fff' }}>📋 AD</span>
                        {item.isNegotiable && (
                          <span style={{ position: 'absolute', bottom: 6, left: 6, fontSize: 9, fontWeight: 800, padding: '2px 6px', borderRadius: 6, backgroundColor: '#fef9c3', color: '#ca8a04' }}>Negotiable</span>
                        )}
                      </div>
