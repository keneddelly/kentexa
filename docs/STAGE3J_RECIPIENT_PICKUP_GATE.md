# Stage 3J — recipient pickup gate

Production baseline: `0a553c4c2e006ef0eb26fd44f3c74e74d5ddbfbb`. The review candidate is PR #35, based on Stage 3I `7ec0936`.
...
The feature branch now adds a bounded completion callback inside canonical seller release. COD code verification rechecks the locked Parcel after preflight and joins custody, tracking, physical status, hub cash liability, agent fee, and invoice/receipt with the release transaction. Seller-arranged shipments use a transaction without a seller credit, preserving the zero-fee routing rule. Unit tests and frontend/backend builds pass. A PostgreSQL test covers release callback rollback, blocked routing, and concurrent release. The original non-COD pickup PostgreSQL test covers custody rollback and replay. A further PostgreSQL test drives the full COD service path using a database-backed release stand-in to prove blocked release, tracking/receipt rollback, replay, and concurrent confirmation. All six CI workflows passed on PR #35 head `e20812285bd8f10cfa59009680d7743534adc188`. Because the stand-in does not prove every canonical release and invoice internal in one test, independent review must assess the combined evidence. PR #35 is ready for independent review; production migration/deployment remains blocked. The legacy generic COD delivery endpoint still has its older split accounting writes and is outside this candidate.

## Author review finding — concurrent status writer

The legacy hub status sheet read a Parcel and later wrote its status and tracking separately. A status request already in flight could overwrite a completed recipient pickup or append newer stale tracking. The candidate now row-locks and rechecks awaiting-choice Parcels, writing status and tracking in one transaction; terminal SELF_PICKUP blocks later generic edits. A Parcel claimed by a local delivery agent is not eligible for hub self-pickup. Focused tests cover stale status and claimed-agent rejection. This correction requires the exact-head PostgreSQL CI gate before release. The older generic COD delivery accounting path remains a separate production debt and does not gain atomicity from this pickup correction.
